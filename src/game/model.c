...
void modelUpdateRelationsQuick(Model *model, ModelNode *parent)
{
    ...
    while (node)
    {
        ...
        switch (type)
        {
            ...
            case MODELNODE_OPCODE_SWITCH:
                /*
                 * Switch child pointers live in the shared model definition.
                 * Re-apply this instance's rwdata before quick matrix updates,
                 * otherwise another instance/viewport can leave the shared
                 * relation in the wrong state. Perfect Dark explicitly follows
                 * this per-instance relation discipline.
                 */
