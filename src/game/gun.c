...
/*
 * Expansion-Pak-backed first-person texture cache.
 *
 * Retail rebuilds a small per-hand texture pool every time a weapon model is
 * loaded, so switching to a weapon whose textures are not already in that
 * temporary pool incurs ROM DMA + texture decompression on the gameplay frame.
 * The physical build has 2 MiB of otherwise-unused Expansion Pak RDRAM below
 * the resident game segment.  Keep decoded first-person textures there for the
 * duration of the stage and let every hand/model share them.
 */
...
static s32 g_WeaponTextureCachePrewarmed = FALSE;
static s32 g_WeaponTextureCacheOverflowed = FALSE;
...
static struct texpool *gunGetFirstPersonTexturePool(void)
{
    if (!g_WeaponTextureCacheReady)
    {
        gunInitFirstPersonTextureCache();
    }

    return &g_WeaponTextureCachePool;
}

extern const s16 g_FirstPersonPrewarmTextureIds[];
#define FIRST_PERSON_PREWARM_TEXTURE_COUNT 605

void gunPrewarmFirstPersonTextureCache(void)
{
    s32 i;
    struct texpool *pool;
     ...
     * PI-DMAing and decompressing every gun/item model merely to rediscover
     * those IDs at stage load.  The set and first-load order are the same as
     * the previous runtime scan, but 83 model-file loads disappear entirely.
     */
    for (i = 0; i < FIRST_PERSON_PREWARM_TEXTURE_COUNT; i++)
    {
        s32 textureword = g_FirstPersonPrewarmTextureIds[i];
        s32 before = texFreeBytesInBuffer(pool);

        texLoad(&textureword, pool);

        /* texLoad's retail out-of-space fallback writes pool->start.  Record
         * that condition so weapon loading can safely use the original
         * per-hand texture pool instead of drawing invalid data. */
        if (texFreeBytesInBuffer(pool) == before &&
            texFindInPool(g_FirstPersonPrewarmTextureIds[i], pool) == NULL)
        {
...
#ifdef GE_PHYSICAL_FASTPATHS
                if (!g_WeaponTextureCacheOverflowed)
                {
                    struct texpool *sharedpool = gunGetFirstPersonTexturePool();

                    if (item == ITEM_SUIT_LF_HAND)
                    {
                        load_object_fill_header(&g_CurrentPlayer->copy_of_body_obj_header[hand], (u8 *)ptr_item_text, buffer_weapon, 0xBD70, sharedpool);
                    }
                    else if ((item == ITEM_TRIGGER) || (item == ITEM_WATCHLASER))
                    {
                        load_object_fill_header(&g_CurrentPlayer->copy_of_body_obj_header[hand], (u8 *)ptr_item_text, buffer_weapon, 0xAFD0, sharedpool);
                    }
                    else
                    {
                        load_object_fill_header(&g_CurrentPlayer->copy_of_body_obj_header[hand], (u8 *)ptr_item_text, buffer_weapon, D_80032464[hand], sharedpool);
                    }
                }
                else
#endif
