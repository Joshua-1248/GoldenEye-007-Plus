...
#ifdef GE_FASTBUILD
s32 stanCheckLinkedSpecialTile(StandTile *tile, s32 pointIdx, f32 arg2, f32 arg3, f32 arg4, struct StandTileLocusCallbackRecord *record);
#else
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
    /*
     * Exact broad-phase rejection. If the two closed segment AABBs do not
     * overlap then the segments cannot intersect, so avoid the substantially
     * more expensive rotational-direction tests below. Strict comparisons
     * deliberately keep touching boxes on the original path. NaNs also fall
     * through because every ordered comparison with NaN is false.
     */
    if (((start1X < end1X ? end1X : start1X) < (start2X < end2X ? start2X : end2X)) ||
        ((start2X < end2X ? end2X : start2X) < (start1X < end1X ? start1X : end1X)) ||
        ((start1Z < end1Z ? end1Z : start1Z) < (start2Z < end2Z ? start2Z : end2Z)) ||
        ((start2Z < end2Z ? end2Z : start2Z) < (start1Z < end1Z ? start1Z : end1Z)))
    {
        return FALSE;
    }
#endif
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
/*
 * A collision query commonly tests one fixed movement segment against every
 * edge of several prop polygons. Retail's doSegmentsIntersect() recomputes
 * the movement segment's bounds and delta for every edge. Keep those values
 * once per query instead. The edge-side calculations and the rotational
 * tests are otherwise exactly the same operations as doSegmentsIntersect().
 */
typedef struct FastSegmentQuery2D {
    f32 startX;
    f32 startZ;
    f32 endX;
    f32 endZ;
...
static void stanInitFastSegmentQuery(FastSegmentQuery2D *query,
                                     f32 startX, f32 startZ,
                                     f32 endX, f32 endZ)
{
    query->startX = startX;
    query->startZ = startZ;
    query->endX = endX;
    query->endZ = endZ;
    query->maxX = startX < endX ? endX : startX;
    query->minX = startX < endX ? startX : endX;
    query->maxZ = startZ < endZ ? endZ : startZ;
    query->minZ = startZ < endZ ? startZ : endZ;
    query->dx = endX - startX;
    query->dz = endZ - startZ;
}

static bool stanFastSegmentIntersectsEdge(const FastSegmentQuery2D *query,
                                          f32 start2X, f32 start2Z,
                                          f32 end2X, f32 end2Z)
{
    f32 start1RelX;
    f32 start1RelZ;
    ...
    /* Same strict closed-AABB test as doSegmentsIntersect(). */
    if ((query->maxX < (start2X < end2X ? start2X : end2X)) ||
        ((start2X < end2X ? end2X : start2X) < query->minX) ||
        (query->maxZ < (start2Z < end2Z ? start2Z : end2Z)) ||
        ((start2Z < end2Z ? end2Z : start2Z) < query->minZ))
    {
        return FALSE;
    }

    start1RelX = query->startX - start2X;
    start1RelZ = query->startZ - start2Z;
    edgeDx = end2X - start2X;
    edgeDz = end2Z - start2Z;

    return
        (
            (getRotationalDirectionBetween(query->dx, query->dz, -start1RelX, -start1RelZ)
            * getRotationalDirectionBetween(query->dx, query->dz, end2X - query->startX, end2Z - query->startZ)) < 1)
        &&
        (
            (getRotationalDirectionBetween(edgeDx, edgeDz, start1RelX, start1RelZ)
            * getRotationalDirectionBetween(edgeDx, edgeDz, query->endX - start2X, query->endZ - start2Z)) < 1)
        ;
}
#endif
...
s32 stanTestLineUnobstructed(StandTile **pTile, f32 p_x, f32 p_z, f32 dest_x, f32 dest_z, s32 cdtypes, f32 unkHeight, f32 unkA, f32 unkB, f32 unkC)
{
    ...
    s32 already_set;
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
    FastSegmentQuery2D fastSegmentQuery;
#endif
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
    stanInitFastSegmentQuery(&fastSegmentQuery, p_x, p_z, dest_x, dest_z);
#endif
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
                        if (stanFastSegmentIntersectsEdge(&fastSegmentQuery, polygon->points[i].f[0], polygon->points[i].f[1], polygon->points[next].f[0], polygon->points[next].f[1]) != 0)
#else
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
                                                /* Retail shortens the tested movement segment here. */
                                                stanInitFastSegmentQuery(&fastSegmentQuery, p_x, p_z, dest_x, dest_z);
#endif
                                            }
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
    stanInitFastSegmentQuery(&fastSegmentQuery, start_x, start_z, end_x, end_z);
#endif
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
                            if (stanFastSegmentIntersectsEdge(&fastSegmentQuery, polygon->points[i].f[0], polygon->points[i].f[1], polygon->points[next].f[0], polygon->points[next].f[1]))
#else
...
StandTile *stanFillSearch(StandTile *starttile, tilePredicate_t predicate) // stanFillSearch is the canonical name for this function
{
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
    /*
     * 1024-bit exact-negative filter for the BFS discovered set. A clear bit
     * proves the tile has not been seen, so the O(n) pointer scan can be
     * skipped. A set bit is only a maybe (different STAN offsets can alias the
     * same bit), so collisions always fall back to the retail pointer scan.
     * This can therefore change work performed, but never discovery order or
     * predicate behaviour.
     */
    u32 seenFilter[32];
    u32 seenFilterKey;
...
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
    /*
     * The retail routine implements breadth-first discovery in "waves", but
     * restarts at bfsTileStack[0] on every wave. Those repeated scans can never
     * discover a tile earlier: every neighbour is still rejected by the same
     * first-seen list. Process the same discovery queue once instead. Predicate
     * calls and first-seen ordering are unchanged.
     */
    ...
    /* STAN links encode (tile - standTileStart) >> 3. */
    seenFilterKey = (u32)(((u8 *)starttile - (u8 *)standTileStart) >> 3);
    seenFilterKey ^= seenFilterKey >> 10;
    seenFilter[seenFilterKey & 31] |= 1U << ((seenFilterKey >> 5) & 31);

    while (stackindex < seenCount)
    {
        starttile = bfsTileStack[stackindex++];
        pointcount = (starttile->tail.half >> 12) & 0xf;

        for (pointindex = 0; pointindex < pointcount; pointindex++)
        {
            link = starttile->points[pointindex].link;
            ...
            /*
             * The low 10 bits of the encoded STAN offset select one of 1024
             * filter bits. Clear => definitely new. Set => possible collision,
             * so retain the original exact pointer scan.
             */
            seenFilterKey = (u32)link;
            seenFilterKey ^= seenFilterKey >> 10;
            seenFilterWord = &seenFilter[seenFilterKey & 31];
            seenFilterMask = 1U << ((seenFilterKey >> 5) & 31);

            if ((*seenFilterWord & seenFilterMask) != 0)
            {
            ...
            bfsTileStack[seenCount] = linkedtile;
            seenCount++;
...
#ifdef GE_FASTBUILD
    /*
     * IDO lowers these 64-bit conversions/multiplies to libultra helpers.
     * Call those helpers explicitly so modern GCC does not introduce libgcc
     * symbols (__fixsfdi/__floatdidf) that the retail link never provided.
     */
    {
        extern s64 __f_to_ll(f32);
...
#ifdef GE_FASTBUILD
    {
        extern f64 __ll_to_d(s64);
        return (f32)(((__ll_to_d(rsum) - ((f64)p_x * __ll_to_d(cp[0])))
            - ((f64)p_z * __ll_to_d(cp[2]))) / __ll_to_d(cp[1])) * inv_level_scale;
    }
#else
...
#if defined(GE_FASTBUILD)
    return (s32)sub_GAME_7F0B4F9C(room);
#else
