...
s32 objTick(struct PropRecord *prop)
{
	...
	else
	{
		...
		if (obj->runtime_bitflags & RUNTIMEBITFLAG_HASPROJECTILE)
		{
			...
			/*
			 * Retail US multiplayer assumes a projectile owner is always a player.
			 * Campaign Co-Op breaks that assumption: objDrop() deliberately leaves a
			 * dead guard as the owner of its dropped weapon.  If we blindly compare
			 * that NPC prop against the current player, no viewport ever becomes the
			 * simulation owner and gravity never advances.  Shooting the floating gun
			 * masks the bug because objBounce() reassigns ownerprop to the shooter.
			 *
			 * JP/EU retail already contain the correct guard.  Apply the same check to
			 * the modded US build: only player-owned projectiles override the normal
			 * first-shuffled-player simulation owner.  NPC-owned drops stay on the
			 * deterministic once-per-frame owner selected above.
			 */
			if (projectile->ownerprop != NULL
#if defined(VERSION_JP) || defined(VERSION_EU) || defined(GE_MODDED_CHEATS)
            ...
            else
            {
                if (bondinvAddWeaponByProp(prop) != 0)
                {
                    ...
                    if (gamemode == GAMEMODE_MULTI && get_scenario() == SCENARIO_COOP
                        && wep->weaponnum >= ITEM_BOMBCASE && wep->weaponnum <= ITEM_KEYBOLT)
                    {
                        bondinvShareCoopItem(wep->weaponnum);
                    }
#endif
                }
...
#ifdef GE_MODDED_CHEATS
        /* Keys and generic mission props are progression state in campaign
         * Co-Op, not personal loot.  Give one pickup to the whole team and
         * remember it across individual respawns. */
        if (gamemode == GAMEMODE_MULTI && get_scenario() == SCENARIO_COOP
            && op == TICKOP_GIVETOPLAYER
            && (obj->type != PROPDEF_COLLECTABLE
                || (((WeaponObjRecord *)obj)->weaponnum >= ITEM_BOMBCASE
                    && ((WeaponObjRecord *)obj)->weaponnum <= ITEM_KEYBOLT)))
        {
            bondinvShareCoopProp(prop);
        }
#endif
...
Gfx *countdownTimerRender(Gfx *DL)
{
    ...
    if (clock_drawn_flag == 0) {
...
#ifdef GE_MODDED_CHEATS
        /* Co-Op owns one mission clock. Draw it once, centred exactly on
         * the full-screen split intersection instead of once per viewport. */
        ...
        {
            xpos = 0xA0;
            ypos = (viGetViewTop() + viGetViewHeight()) - valign_offset;
        }

        // Minutes
        DL = gunDrawHudInteger(DL, (mins % 100) / 10, xpos - 30, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);
        DL = gunDrawHudInteger(DL, mins % 10, xpos - 22, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);

        // :
        DL = gunDrawHudString(DL, &D_80052A44, xpos - 13, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);

        // Seconds
        DL = gunDrawHudInteger(DL, (secs % 60) / 10, xpos - 4, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);
        DL = gunDrawHudInteger(DL, secs % 10, xpos + 4, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);

        // :
        DL = gunDrawHudString(DL, &D_80052A44, xpos + 13, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);

        // Milliseconds
        DL = gunDrawHudInteger(DL, (ms % 100) / 10, xpos + 22, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);
        DL = gunDrawHudInteger(DL, ms % 10, xpos + 30, HUDHALIGN_MIDDLE, ypos, HUDVALIGN_MIDDLE, 1);
