...
void chraiUpdateOnscreenPropCount(void)
{
    ...
    {
        /*
         * This depth list is rebuilt for every viewport. Retail uses a full
         * selection sort (N + N-1 + ... comparisons).  Co-Op makes that cost
         * scale directly with player count, so use an in-place insertion sort
         * instead.  Typical onscreen lists are already close to depth order
         * from the previous viewport/frame, making this substantially cheaper.
         * Equal-depth entries keep their existing order.
         */
        for (i = 1; i < count; i++)
        {
            sortprop = g_OnScreenPropList[i];
            sortdepth = sortprop->zDepth;
            k = i - 1;

            while (k >= 0 && g_OnScreenPropList[k]->zDepth < sortdepth)
            {
                g_OnScreenPropList[k + 1] = g_OnScreenPropList[k];
                k--;
            }

            g_OnScreenPropList[k + 1] = sortprop;
        }
    }
    else
#endif
...
s32 chrpropFindFirstBgHitInConnectedRooms(s32 startroom, coord3d *from, coord3d *to, coord3d *dir, coord3d *scaledDir, u8 *visited, struct HitThing *hit)
{
    ...
    queued[((u32) startroom & 0xff) >> 5] |= 1u << (((u32) startroom & 0xff) & 31);
#endif
    ...
    for (ptr = g_LastOnScreenProp - 1; ptr >= g_OnScreenPropList; ptr--)
    {
        ...
        if (prop)
        {
            if (prop->type == PROP_TYPE_CHR)
            {
                ...
                /* R21 campaign Co-Op escort ownership is a real B-use
                 * interaction.  Use the same basic reach/facing/LOS envelope
                 * as ordinary activatable props rather than assigning an NPC
                 * merely because a player happens to walk nearby. */
                if (gamemode == GAMEMODE_MULTI
                    && get_scenario() == SCENARIO_COOP
                    && chr
                    && chr->actiontype != ACT_DIE
                    && chr->actiontype != ACT_DEAD
                    && chrCoopIsEscortCandidate(chr))
                {
                    PropRecord *player = getCurrentPlayerProp();
                    f32 xdiff = prop->pos.x - player->pos.x;
                    f32 ydiff = prop->pos.y - player->pos.y;
                    f32 zdiff = prop->pos.z - player->pos.z;
                    f32 xzdiff = xdiff * xdiff + zdiff * zdiff;

                    if (xzdiff < 40000.0f && ydiff < 200.0f && ydiff > -200.0f)
                    {
                        f32 angle = atan2f(xdiff, zdiff);
                        f32 playerangle = bondviewGetPlayerYawRadians();
                        f32 anglediff = angle - playerangle;
                        ...
                        if (anglediff <= 0.3926991f
                            && walkTilesBetweenPoints_NoCallback(&player->stan,
        ...
        switch (prop->type)
        {
            ...
            case PROP_TYPE_CHR:
                ...
                if (gamemode == GAMEMODE_MULTI
                    && get_scenario() == SCENARIO_COOP
                    && prop->chr
                    && chrCoopIsEscortCandidate(prop->chr))
                {
                    chrCoopAssignEscortOwner(prop->chr, get_cur_playernum());
                }
#endif
                break;
...
void roomGetProps(s32 *rooms)
{
    ...
    /*
     * A prop may be present in several room lists. The original code keeps
     * output order stable by linearly searching every prop already emitted,
     * making collection O(n^2) in the number of candidate props. A 600-bit
     * local set makes the duplicate test O(1) while preserving the exact
     * first-seen output order. Invalid/corrupt indices retain the original
     * fallback search so this fast path does not broaden assumptions.
     */
    u32 seenProps[(MAX_PROPS + 31) / 32];

    for (i = 0; i < (s32)(sizeof(seenProps) / sizeof(seenProps[0])); i++)
    {
        seenProps[i] = 0;
    }
#endif
    ...
    while (room >= 0)
    {
        ...
        while (chunkindex >= 0)
        {
            ...
            for (i = 0; i < 15; i++)
            {
                ...
                {
#if defined(GE_FASTBUILD) || defined(GE_OPTIMIZED)
                    if ((u32)propnum < MAX_PROPS)
                    {
                        u32 mask = 1U << (propnum & 31);
                        u32 *word = &seenProps[(u32)propnum >> 5];
