...
#define GE_EXP_ROOM_SCALE() roomscale
#else
#define GE_EXP_ROOM_SCALE() get_room_data_float1()
#endif
...
s32 explosionTick(PropRecord* arg0)
{
...
#ifdef GE_PHYSICAL_FASTPATHS
        /*
         * Retail restarts the 40-slot scan at index zero for each new flare.
         * No slot can become free during this loop, so the first-free indices
         * are monotonically increasing.  Continue from the previous hit and
         * preserve the exact slot order and RNG call order.
         */
        j = 0;
        for (var_s4 = 0; var_s4 < sp9C; var_s4++)
        {
            while (j < EXPLOSION_PARTS_LEN && exp->parts[j].frame != 0)
            {
                j++;
            }

            if (j >= EXPLOSION_PARTS_LEN)
            {
                break;
            }
...
#ifdef GE_PHYSICAL_FASTPATHS
    /*
     * Part frames are not consulted by the age-triggered smoke/scorch events
     * below. Retail increments every active part once per logical update;
     * defer that independent integer work until after the age loop so a lag
     * catch-up of up to 15 ticks scans the 40 parts once rather than 15 times.
     */
    ...
    for (j = 0; j < EXPLOSION_PARTS_LEN; j++)
    {
        if (exp->parts[j].frame > 0)
        {
            exp->parts[j].frame += (s32)lvupdate;
        }
    }
#endif
...
Gfx *explosionRenderPropExplosion(PropRecord *prop, Gfx *gdl, s32 withalpha)
{
    ...
    else
    {
...
#ifdef GE_PHYSICAL_FASTPATHS
        /*
         * Retail evaluates the flare-frame floating-point division once for
         * every (animation frame x active part) pair: up to 15 * 40 = 600
         * divisions per explosion per rendered frame. The result depends only
         * on the part and explosion type, so calculate it once per part while
         * retaining the original frame-descending / part-ascending draw order.
         */
        ...
        /*
         * R8: classify every active flare exactly once, then render each
         * animation bucket in the retail frame-descending / part-ascending
         * order. R6 removed up to 600 divisions but still performed the full
         * 15 x 40 comparison grid. This reduces that grid to 40 classifications
         * plus one visit for each actually rendered flare.
         */
        ...
        for (i = 0; i < EXPLOSION_PARTS_LEN; i++)
        {
            if (temp_s5->parts[i].frame > 0)
            {
                ...
                if ((u32)bucket < 15U)
                {
                    ...
                    {
                        bucketnext[(s32)buckettail[bucket]] = i;
                    }
                    buckettail[bucket] = i;
                }
            }
        ...
        for (var_s2 = 14; var_s2 >= 0; var_s2--)
        {
            ...
            {
                gdl = explosionRenderPartFast(&temp_s5->parts[i], gdl, temp_s6,
                    fastviewtoworld, fastplayerpos, fastroomscale);
            }
        }
#else
        for (var_s2 = 14;
...
#ifdef GE_PHYSICAL_FASTPATHS
        temp_f10 = (s32) (explosiontype->flareanimspeed * 15.0f);
#else
...
#ifdef GE_PHYSICAL_FASTPATHS
/*
 * R9 close-range flare renderer.
 *
 * For distances 0 < d <= 200 the retail expression is exactly:
 *   range = d * 0.5; mult = (d - range) / d = 0.5
 * so sqrtf is unnecessary.  This is precisely the case that hurts most when
 * several explosions fill the player's view.  Farther flares retain the
 * original sqrtf path.  Player/view/room state is supplied once per explosion
 * instead of being looked up again for every flare.
 */
Gfx *explosionRenderPartFast(struct ExplosionPart *part, Gfx *gdl, struct coord3d *roompos,
        Mtxf *viewtoworld, struct coord3d *playerpos, f32 roomscale)
{
    Vtx base = g_ExplosionRenderPartDefaultVertex;
    Vtx *vertices;
    struct coord3d size_x;
    struct coord3d rot_x;
    struct coord3d size_y;
    struct coord3d rot_y;
    f32 dx;
    f32 dy;
    f32 dz;
    f32 dist2;
    ...
    dx = part->pos.f[0] - playerpos->f[0];
    dy = part->pos.f[1] - playerpos->f[1];
    dz = part->pos.f[2] - playerpos->f[2];
    ...
    else
    {
        ...
        mult = (dist - 100.0f) / dist;
    }

    scaledsize = part->size * mult;
    scaledrot = part->rot * mult;

    cx = playerpos->f[0] + dx * mult;
    cy = playerpos->f[1] + dy * mult;
    cz = playerpos->f[2] + dz * mult;

    size_x.f[0] = viewtoworld->m[0][0] * scaledsize;
    size_x.f[1] = viewtoworld->m[0][1] * scaledsize;
    size_x.f[2] = viewtoworld->m[0][2] * scaledsize;
    rot_x.f[0] = viewtoworld->m[0][0] * scaledrot;
    rot_x.f[1] = viewtoworld->m[0][1] * scaledrot;
    rot_x.f[2] = viewtoworld->m[0][2] * scaledrot;
    size_y.f[0] = viewtoworld->m[1][0] * scaledsize;
    size_y.f[1] = viewtoworld->m[1][1] * scaledsize;
    size_y.f[2] = viewtoworld->m[1][2] * scaledsize;
    rot_y.f[0] = viewtoworld->m[1][0] * scaledrot;
    ...
    vertices[0].v.ob[2] = (cz - size_x.f[2] - rot_y.f[2]) * roomscale - roompos->f[2];
    vertices[0].v.tc[0] = 1760;
    vertices[0].v.tc[1] = 0;

    vertices[1].v.ob[0] = (cx + rot_x.f[0] - size_y.f[0]) * roomscale - roompos->f[0];
    vertices[1].v.ob[1] = (cy + rot_x.f[1] - size_y.f[1]) * roomscale - roompos->f[1];
    vertices[1].v.ob[2] = (cz + rot_x.f[2] - size_y.f[2]) * roomscale - roompos->f[2];
    vertices[1].v.tc[0] = 0;
    vertices[1].v.tc[1] = 0;

    vertices[2].v.ob[0] = (cx + size_x.f[0] + rot_y.f[0]) * roomscale - roompos->f[0];
    vertices[2].v.ob[1] = (cy + size_x.f[1] + rot_y.f[1]) * roomscale - roompos->f[1];
    vertices[2].v.ob[2] = (cz + size_x.f[2] + rot_y.f[2]) * roomscale - roompos->f[2];
    vertices[2].v.tc[0] = 0;
    vertices[2].v.tc[1] = 1760;

    vertices[3].v.ob[0] = (cx - rot_x.f[0] + size_y.f[0]) * roomscale - roompos->f[0];
    vertices[3].v.ob[1] = (cy - rot_x.f[1] + size_y.f[1]) * roomscale - roompos->f[1];
    vertices[3].v.ob[2] = (cz - rot_x.f[2] + size_y.f[2]) * roomscale - roompos->f[2];
...
Gfx *explosionRenderPart(struct ExplosionPart *arg0, Gfx *gdl, struct coord3d *coord)
{
    ...
    vertices[0].v.ob[0] = (sp4c - sp8C.f[0] - sp68.f[0]) * GE_EXP_ROOM_SCALE() - coord->f[0];
	vertices[0].v.ob[1] = (sp48 - sp8C.f[1] - sp68.f[1]) * GE_EXP_ROOM_SCALE() - coord->f[1];
	vertices[0].v.ob[2] = (sp44 - sp8C.f[2] - sp68.f[2]) * GE_EXP_ROOM_SCALE() - coord->f[2];
	vertices[0].v.tc[0] = 1760;
	vertices[0].v.tc[1] = 0;

	vertices[1].v.ob[0] = (sp4c + sp80.f[0] - sp74.f[0]) * GE_EXP_ROOM_SCALE() - coord->f[0];
	vertices[1].v.ob[1] = (sp48 + sp80.f[1] - sp74.f[1]) * GE_EXP_ROOM_SCALE() - coord->f[1];
	vertices[1].v.ob[2] = (sp44 + sp80.f[2] - sp74.f[2]) * GE_EXP_ROOM_SCALE() - coord->f[2];
	vertices[1].v.tc[0] = 0;
	vertices[1].v.tc[1] = 0;

	vertices[2].v.ob[0] = (sp4c + sp8C.f[0] + sp68.f[0]) * GE_EXP_ROOM_SCALE() - coord->f[0];
	vertices[2].v.ob[1] = (sp48 + sp8C.f[1] + sp68.f[1]) * GE_EXP_ROOM_SCALE() - coord->f[1];
	vertices[2].v.ob[2] = (sp44 + sp8C.f[2] + sp68.f[2]) * GE_EXP_ROOM_SCALE() - coord->f[2];
	vertices[2].v.tc[0] = 0;
	vertices[2].v.tc[1] = 1760;

	vertices[3].v.ob[0] = (sp4c - sp80.f[0] + sp74.f[0]) * GE_EXP_ROOM_SCALE() - coord->f[0];
	vertices[3].v.ob[1] = (sp48 - sp80.f[1] + sp74.f[1]) * GE_EXP_ROOM_SCALE() - coord->f[1];
	vertices[3].v.ob[2] = (sp44 - sp80.f[2] + sp74.f[2]) * GE_EXP_ROOM_SCALE() - coord->f[2];
	vertices[3].v.tc[0] = 1760;
	vertices[3].v.tc[1] = 1760;
...
*/
#ifdef GE_PHYSICAL_FASTPATHS
#define GE_SMOKE_ROOM_SCALE() roomscale
#define GE_SMOKE_TYPE()       (*smoketype)
#else
#define GE_SMOKE_ROOM_SCALE() get_room_data_float1()
...
Gfx *explosionSmokeRenderPart(struct Smoke *smoke, struct SmokePart *smoke_part, Gfx *gdl, struct coord3d *arg3)
{
    ...
    {
        sp77 = (smoke_part->alpha / (f32) GE_SMOKE_TYPE().rateappear) * smoke_part->count;
    }
    else
    {
    ...
    sp54 = ((sp80 - spB0.f[0] - sp8C.f[0]) * GE_SMOKE_ROOM_SCALE() - arg3->f[0]) * 10.0f;
	sp50 = ((sp7C - spB0.f[1] - sp8C.f[1]) * GE_SMOKE_ROOM_SCALE() - arg3->f[1]) * 10.0f;
	sp4C = ((sp78 - spB0.f[2] - sp8C.f[2]) * GE_SMOKE_ROOM_SCALE() - arg3->f[2]) * 10.0f;
	...
	vertices[0].v.tc[0] = 1760;
	vertices[0].v.tc[1] = 0;
    vertices[0].v.cn[0] = GE_SMOKE_TYPE().r;
    vertices[0].v.cn[1] = GE_SMOKE_TYPE().g;
    vertices[0].v.cn[2] = GE_SMOKE_TYPE().b;
    vertices[0].v.cn[3] = sp77;

    vertices[1].v.ob[0] = ((((sp80 + spA4.f[0]) - sp98.f[0]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[0]) * 10.0f;
    vertices[1].v.ob[1] = ((((sp7C + spA4.f[1]) - sp98.f[1]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[1]) * 10.0f;
    vertices[1].v.ob[2] = ((((sp78 + spA4.f[2]) - sp98.f[2]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[2]) * 10.0f;
    vertices[1].v.tc[0] = 0;
    vertices[1].v.tc[1] = 0;
    vertices[1].v.cn[0] = GE_SMOKE_TYPE().r;
    vertices[1].v.cn[1] = GE_SMOKE_TYPE().g;
    vertices[1].v.cn[2] = GE_SMOKE_TYPE().b;
    vertices[1].v.cn[3] = sp77;

    vertices[2].v.ob[0] = (((sp80 + spB0.f[0] + sp8C.f[0]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[0]) * 10.0f;
    vertices[2].v.ob[1] = (((sp7C + spB0.f[1] + sp8C.f[1]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[1]) * 10.0f;
    vertices[2].v.ob[2] = (((sp78 + spB0.f[2] + sp8C.f[2]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[2]) * 10.0f;
    vertices[2].v.tc[0] = 0;
    vertices[2].v.tc[1] = 1760;
    vertices[2].v.cn[0] = GE_SMOKE_TYPE().r;
    vertices[2].v.cn[1] = GE_SMOKE_TYPE().g;
    vertices[2].v.cn[2] = GE_SMOKE_TYPE().b;
    vertices[2].v.cn[3] = sp77;

    vertices[3].v.ob[0] = ((((sp80 - spA4.f[0]) + sp98.f[0]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[0]) * 10.0f;
    vertices[3].v.ob[1] = ((((sp7C - spA4.f[1]) + sp98.f[1]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[1]) * 10.0f;
    vertices[3].v.ob[2] = ((((sp78 - spA4.f[2]) + sp98.f[2]) * GE_SMOKE_ROOM_SCALE()) - arg3->f[2]) * 10.0f;
    vertices[3].v.tc[0] = 1760;
    vertices[3].v.tc[1] = 1760;
    vertices[3].v.cn[0] = GE_SMOKE_TYPE().r;
    vertices[3].v.cn[1] = GE_SMOKE_TYPE().g;
    vertices[3].v.cn[2] = GE_SMOKE_TYPE().b;
