...
/*
 * R18: The level is torn down before the folder frontend draws campaign
 * results.  Preserve each Co-Op player's final counters while their player
 * records are still valid; single-player continues to use the retail path.
 */
struct CoopMissionReportSnapshot
{
    s32 valid;
    s32 playercount;
    s32 missionfailed;
    ...
    s32 killcount[MAX_PLAYER_COUNT];
    s32 favoriteweapon[MAX_PLAYER_COUNT][GUNHANDS];
};
...
#ifdef GE_MODDED_CHEATS
    {0, 2, 4}  // Co-Op (literal frontend label; campaign mission flow)
#endif
};
...
struct mp_stage_setup multi_stage_setups[] = {
    ...
    {getStringID(LTITLE, TITLE_STR_166_SEVERNAYABUNKER), getStringID(LTITLE, TITLE_STR_167_BUNKER), IMG_MP_BUNKER2, LEVELID_BUNKER2, SP_LEVEL_BUNKER2, 1, 4},
    {getStringID(LTITLE, TITLE_STR_168_MILITARYARCHIVES), getStringID(LTITLE, TITLE_STR_169_ARCHIVES), IMG_MP_ARCHIVES, LEVELID_ARCHIVES, SP_LEVEL_ARCHIVES, 1, 4},
    {getStringID(LTITLE, TITLE_STR_170_WATERCAVERNS), getStringID(LTITLE, TITLE_STR_171_CAVERNS), IMG_MP_CAVERNS, LEVELID_CAVERNS, SP_LEVEL_CAVERNS, 1, 4},
...
#ifdef GE_MODDED_CHEATS
static u8 *frontGetMpScenarioText(s32 scenarioid)
{
    if (scenarioid == SCENARIO_COOP)
    {
        return (u8 *)"Co-Op";
    }
    return langGet(mp_player_counts[scenarioid].stage);
}

static s32 frontMpScenarioAllowedForCount(s32 scenarioid, s32 playercount)
{
    if (scenarioid == SCENARIO_COOP)
    {
    ...
    return playercount >= mp_player_counts[scenarioid].min
        && playercount <= mp_player_counts[scenarioid].max;
}

/* Keep the engine scenario IDs stable while presenting Co-Op immediately
 * after Normal in the frontend list. */
static s32 frontMpScenarioForDisplaySlot(s32 slot)
{
    if (slot == 1)
    {
        return SCENARIO_COOP;
    }
    return slot > 1 ? slot - 1 : slot;
}
#endif
...
void frontStoreCoopMissionReportStats(void)
{
    ...
    /* RC6: teardown can reset multiplayer mode/scenario before the late
     * unload hook runs. Never erase an already-captured Co-Op result merely
     * because that late hook no longer sees SCENARIO_COOP. A new mission
     * explicitly clears the snapshot in init_menu0B_runstage. */
    if (gamemode != GAMEMODE_MULTI || scenario != SCENARIO_COOP)
    {
        return;
    }
    ...
    /* Capture weapon-of-choice while the live inventories still exist. */
    set_favorite_weapon_for_every_player();

    g_CoopMissionReport.playercount = count;
    ...
    {
        g_CoopMissionReport.objectivestatus[objective] = get_status_of_objective(objective);
    }

    g_CoopMissionReport.success = !g_CoopMissionReport.missionfailed
        && !g_CoopMissionReport.bondkia;

    if (g_CoopMissionReport.success)
    {
        for (objective = 0; objective < OBJECTIVES_MAX; objective++)
        {
            if (ptrbriefingdata->objective[objective].textid != 0
                && selected_difficulty >= ptrbriefingdata->objective[objective].enabled_difficulty
                && g_CoopMissionReport.objectivestatus[objective] != OBJECTIVESTATUS_COMPLETE)
            {
                g_CoopMissionReport.success = FALSE;
                break;
            }
        }
    }
    ...
    {
        g_CoopMissionReport.deaths[player] =
            g_playerPointers[player] ? g_playerPointers[player]->deathcount : 0;
        g_CoopMissionReport.killcount[player] = g_playerPlayerData[player].kill_count;

        for (reg = 0; reg < 7; reg++)
        {
            g_CoopMissionReport.shotcount[player][reg] = g_playerPlayerData[player].shot_count[reg];
        }

        for (hand = 0; hand < GUNHANDS; hand++)
        {
            g_CoopMissionReport.favoriteweapon[player][hand] = array_favweapon[player][hand];
        }
    }

    g_CoopMissionReport.valid = TRUE;
}

static s32 frontHasCoopMissionReport(void)
{
    return g_CoopMissionReport.valid && g_CoopMissionReport.playercount > 0;
}

static OBJECTIVESTATUS frontGetMissionReportObjectiveStatus(s32 objective)
{
    if (frontHasCoopMissionReport() && objective >= 0 && objective < OBJECTIVES_MAX)
    {
        return g_CoopMissionReport.objectivestatus[objective];
    }

    return get_status_of_objective(objective);
}
#endif
...
#ifdef GE_MODDED_CHEATS
/*
 * Frontend audio helpers for the save-backed Options page.  The retail
 * music setting is stored as an 8-bit value but the audio engine uses a
 * 0..0x7fff volume.  Keep Track 2 (the value used by gameplay) and the
 * currently playing Track 1 (the folder/frontend music) in lockstep.
 */
static u16 frontModAudioVolumeFromSaveValue(u8 value)
{
    return (value << 7) | (value >> 1);
}

static void frontModApplyMusicVolume(save_data *save)
{
    u16 volume;
    ...
    volume = frontModAudioVolumeFromSaveValue(save->music_vol);
    set_mTrack2Vol(volume);
    musicTrack1ApplySeqpVol(volume);
}

static void frontModApplySfxVolume(save_data *save)
{
    if (!save)
    {
        return;
    ...
    sub_GAME_7F0A91A0(frontModAudioVolumeFromSaveValue(save->sfx_vol));
}

static void frontModApplyMenuAudioFromSave(void)
{
...
#ifdef GE_MODDED_CHEATS
    /* R21 RC10: appended mod-only cheats (No Reload, Rapid Fire) have no
     * retail save unlock condition.  Do not feed them into the retail
     * unlock resolver, whose default path intentionally loops forever. */
    for (i=CHEAT_EXTRA_MP_CHARS; i != CHEAT_NO_RELOAD; i++)
#else
...
#ifdef GE_MODDED_CHEATS
    /* The mod branch always exposes Cheat Options because Page 1 contains
     * Line Mode and Page 2 contains the preserved hidden/unused cheats. */
    is_cheat_menu_available = TRUE;
#endif
    viSetFovY(FOV_Y_F);
    viSetAspect(ASPECT_RATIO_SD);
...
#ifdef GE_MODDED_CHEATS
    else if (286.0f <= cursor_v_pos)
    {
        mission_difficulty_highlighted = 3;
        if (joyGetButtonsPressedThisFrame(PLAYER_1, START_BUTTON|Z_TRIG|A_BUTTON))
        {
...
#endif
    else if ((is_cheat_menu_available) && (262.0f <= cursor_v_pos))
    {
...
Gfx* constructor_menu06_modesel(Gfx* DL)
{
    ...
    {
        DL = microcode_constructor_related_to_menus(DL, 0x94, 0xF2, y2 + 0xAF, 0x102, 0x32);
    }
    ...
    if (is_cheat_menu_available != 0)
    {
        ...
        {
            DL = microcode_constructor_related_to_menus(DL, 0x94, 0x10A, y2 + 0xAF, 0x11A, 0x32);
        }
        DL = frontPrintText(DL, &x, &y, textstring, ptrFontZurichBoldChars, ptrFontZurichBold, 0xFF, viGetX(), viGetY(), 0, 0);
    }
    ...
    x = 0xAA;
    y = 0x124;
    if (mission_difficulty_highlighted == 3)
    {
        DL = microcode_constructor_related_to_menus(DL, 0x94, 0x122, y2 + 0xAF, 0x132, 0x32);
    }
    DL = frontPrintText(DL, &x, &y, textstring, ptrFontZurichBoldChars, ptrFontZurichBold, 0xFF, viGetX(), viGetY(), 0, 0);
#endif
...
#define MISSION_SELECT_STAGE_COUNT 20

static const s8 g_MissionSelectEntryByStage[MISSION_SELECT_STAGE_COUNT] = {
    1, 2, 3, 5, 6, 8, 10, 12, 13, 15,
    16, 17, 18, 19, 21, 22, 23, 24, 26, 28
};

static s8 g_MissionSelectUnlockedDifficulty[MISSION_SELECT_STAGE_COUNT];
static u8 g_MissionSelectLabel[MISSION_SELECT_STAGE_COUNT][24];
static s16 g_MissionSelectLabelWidth[MISSION_SELECT_STAGE_COUNT];
static s16 g_MissionSelectLabelHeight[MISSION_SELECT_STAGE_COUNT];
static s32 g_MissionSelectLabelsValid;

static s32 missionSelectGetHighestUnlockedDifficulty(s32 stage, s32 maxdifficulty)
{
    s32 difficulty;
    s32 unlocked;

    if ((u32)stage >= MISSION_SELECT_STAGE_COUNT)
    {
        return -1;
    }

    if (mission_folder_setup_entries[g_MissionSelectEntryByStage[stage]].stage_id < 0)
    {
        return -1;
    }

    for (difficulty = maxdifficulty; difficulty >= 0; difficulty--)
    {
        unlocked = fileIsStageUnlockedAtDifficulty(selected_folder_num, stage, difficulty);

        if (g_AppendCheatSinglePlayer == 0)
        {
            if (unlocked != DIFFICULTY_AGENT)
            {
                return difficulty;
            }
        }
        else if (unlocked == DIFFICULTY_007)
        {
            return difficulty;
        }
    }
...
static void missionSelectRefreshUnlockCache(void)
{
    s32 stage;
    s32 maxdifficulty = DIFFICULTY_00;

    if (fileIs007ModeUnlocked(selected_folder_num) || get_debug_007_unlock_flag())
    {
        maxdifficulty = DIFFICULTY_007;
    }

    for (stage = 0; stage < MISSION_SELECT_STAGE_COUNT; stage++)
    {
        g_MissionSelectUnlockedDifficulty[stage] =
            missionSelectGetHighestUnlockedDifficulty(stage, maxdifficulty);
    }
}

static void missionSelectPrepareLabels(void)
{
    s32 stage;
    s32 entry;
    s32 i;
    ...
    for (stage = 0; stage < MISSION_SELECT_STAGE_COUNT; stage++)
    {
        entry = g_MissionSelectEntryByStage[stage];

        if (mission_folder_setup_entries[entry].icon_text_preset)
        {
            text = langGet(mission_folder_setup_entries[entry].icon_text_preset);
        }
        else
        {
            text = langGet(mission_folder_setup_entries[entry].folder_text_preset);
        }

        strcpy(g_MissionSelectLabel[stage], text);

        if (j_text_trigger == 0)
        {
            for (i = strlen(g_MissionSelectLabel[stage]) - 1; i >= 0; i--)
            {
                if (g_MissionSelectLabel[stage][i] >= 'a')
                {
                    g_MissionSelectLabel[stage][i] -= 0x20;
                }
            }
        }

        strcat(g_MissionSelectLabel[stage], "\n");
        width = 0;
        height = 0;
        textMeasure(&height, &width, g_MissionSelectLabel[stage],
            ptrFontBankGothicChars, ptrFontBankGothic, 0);
        g_MissionSelectLabelWidth[stage] = width;
        g_MissionSelectLabelHeight[stage] = height;
    }

    g_MissionSelectLabelsValid = TRUE;
}
#endif
...
{
#ifdef GE_PHYSICAL_FASTPATHS
    if ((u32)arg0 < MISSION_SELECT_STAGE_COUNT)
    {
        return g_MissionSelectEntryByStage[arg0];
    }
    return -1;
#else
    int i;
...
s32 get_highest_unlocked_difficulty_for_level(s32 arg0)
{
    ...
    return missionSelectGetHighestUnlockedDifficulty(arg0, maxdifficulty);
#else
    s32 num;
    s32 difficulty;
    s32 temp_v0;
...
void init_menu07_missionselect(void)
{
    ...
    {
#ifdef GE_MODDED_CHEATS
        frontModApplyMenuAudioFromSave();
#endif
        musicTrack1Play(M_FOLDERS);
        maybe_is_in_menu = FALSE;
    }
    load_walletbond();
#ifdef GE_PHYSICAL_FASTPATHS
    g_MissionSelectLabelsValid = FALSE;
    missionSelectRefreshUnlockCache();
#endif
}
...
void interface_menu07_missionsel(void)
{
    ...
    else
    {
        ...
        for ( ; var_s2 > 0; var_s2--)
        {
            ...
            {
#ifdef GE_PHYSICAL_FASTPATHS
                if (g_MissionSelectUnlockedDifficulty[(var_s2 * 5) + var_s1] >= 0)
#else
        ...
        {
#ifdef GE_PHYSICAL_FASTPATHS
            if (g_MissionSelectUnlockedDifficulty[(var_s2 * 5) + var_s4] >= 0)
#else
...
#ifdef GE_PHYSICAL_FASTPATHS
            if ((u32)temp_a0 < MISSION_SELECT_STAGE_COUNT &&
                g_MissionSelectUnlockedDifficulty[temp_a0] >= 0)
#else
...
Gfx *constructor_menu07_missionsel(Gfx *DL)
{
    ...
    for (spD8 = 0; spD8 < 5; spD8++)
    {
        ...
        {
#ifdef GE_PHYSICAL_FASTPATHS
            temp_s0 = g_MissionSelectEntryByStage[var_s3];
            if (g_MissionSelectUnlockedDifficulty[var_s3] >= 0)
#else
...
#ifdef GE_PHYSICAL_FASTPATHS
                if (!g_MissionSelectLabelsValid)
                {
                    missionSelectPrepareLabels();
                }
                strcpy(sp90, g_MissionSelectLabel[var_s3]);
                spC4 = g_MissionSelectLabelWidth[var_s3];
                spC8 = g_MissionSelectLabelHeight[var_s3];
#else
...
#ifdef GE_MODDED_CHEATS
        p->autoaim = (g_MpPlayerOptions[i] & MP_PLAYEROPT_AUTOAIM) != 0;
        p->sight   = (g_MpPlayerOptions[i] & MP_PLAYEROPT_SIGHT) != 0;
#else
...
#ifdef GE_MODDED_CHEATS
    case SCENARIO_COOP:
        game_length = 0;             /* Unlimited */
        setMPWeaponSet(3);           /* Automatics */
        for (i = 0; i < MAX_PLAYER_COUNT; i++)
        {
            player_handicap[i] = 5;  /* Health +0 */
            player_char[i] = 0;      /* Bond; body/head are stage-aware in bondview2 */
        }
        unlock_stage_select = 0;     /* mission is chosen through solo mission select */
...
void init_mp_options_for_scenario(s32 numplayers)
{
    ...
    if (
#ifdef GE_MODDED_CHEATS
        !frontMpScenarioAllowedForCount(scenario, selected_num_players)
#else
        (mp_player_counts[scenario].max < selected_num_players) || (selected_num_players < mp_player_counts[scenario].min)
#endif
    )
    {
...
void interface_menu0E_mpoptions(void)
{
    ...
    if (tab_start_selected)
    {
        ...
        {
            /* Keep GAMEMODE_MULTI so boss.c constructs selected_num_players, but
             * use the normal campaign mission/difficulty/briefing frontend. */
            briefingpage = -1;
            frontChangeMenu(MENU_MISSION_SELECT, FALSE);
            set_cursor_to_stage_solo(0);
            return;
    ...
    {
        s32 tmpNumPlayers;
        s32 connected = joyGetControllerCount();
...
#ifdef GE_MODDED_CHEATS
/* Multiplayer Settings hub. Health and Control Style keep their existing
 * per-player editors. Player Options replaces the old global Sight/Auto Aim
 * preset with independent P1-P4 control/HUD preferences. No Radar remains an
 * unlock-gated multiplayer rule rather than a visible Cheat Options entry. */
void init_menu_mp_settings(void)
{
    tab_prev_selected = FALSE;
    tab_prev_highlight = FALSE;
    highlight_health = FALSE;
...
void update_menu_mp_settings(void)
{
    return;
}

void interface_menu_mp_settings(void)
{
    s32 selected = 0;
    ...
    if (joyGetButtons(PLAYER_1, Z_TRIG|A_BUTTON) == 0)
    {
        ...
        else if (unlock_handicap)
        {
            highlight_health = TRUE;
        }
    }

    if (joyGetButtonsPressedThisFrame(PLAYER_1, Z_TRIG|A_BUTTON))
    {
        if (tab_prev_highlight)
        {
            tab_prev_selected = TRUE;
    ...
    if (selected)
    {
        ...
        {
            frontChangeMenu(MENU_MP_PLAYER_OPTIONS, FALSE);
        }
        else if (highlight_noradar)
        {
...
Gfx *constructor_menu_mp_settings(Gfx *DL)
{
    u8 *text;
    s32 x;
    s32 y;
 ...
 * active participant slots; A on any other row toggles that player's value. */
void init_menu_mp_player_options(void)
{
    tab_prev_selected = FALSE;
    tab_prev_highlight = FALSE;
    mp_player_options_highlight = 0;
    if (mp_player_options_player >= selected_num_players)
    {
...
void update_menu_mp_player_options(void)
{
    return;
}

void interface_menu_mp_player_options(void)
{
    s32 row;
    ...
    if (joyGetButtonsPressedThisFrame(PLAYER_1, Z_TRIG|A_BUTTON))
    {
        ...
        else if (mp_player_options_highlight == 0)
        {
            mp_player_options_player++;
            if (mp_player_options_player >= selected_num_players)
            {
                mp_player_options_player = 0;
            }
        ...
        {
            g_MpPlayerCrosshair[mp_player_options_player] ^= 1;
        }
        else
        {
            g_MpPlayerOptions[mp_player_options_player] ^= 1 << (mp_player_options_highlight - 1);
            copy_aim_settings_to_playerdata();
        }
        sndPlaySfx((struct ALBankAlt_s *) g_musicSfxBufferPtr, DOOR_METAL_CLOSE2_SFX, NULL);
    }
    ...
    if (tab_prev_selected)
    {
        frontChangeMenu(MENU_MP_SETTINGS, FALSE);
    }
}

Gfx *constructor_menu_mp_player_options(Gfx *DL)
{
    static const u8 optionindex[] = {
        PLAYER_OPTION_LOOK, PLAYER_OPTION_AUTOAIM, PLAYER_OPTION_AIM,
        PLAYER_OPTION_SIGHT, PLAYER_OPTION_LOOKAHEAD, PLAYER_OPTION_AMMODISPLAY
    };
    u8 *text;
    u8 flags;
    s32 x;
    ...
    for (row = 0; row < 10; row++)
    {
        ...
        {
            text = (u8 *)"Player";
            sprintf(value, "P%d", mp_player_options_player + 1);
        }
        else if (row <= 6)
        {
            static const u8 labelindex[6] = { 3, 4, 5, 6, 7, 9 };
            text = (u8 *)frontModGetOptionLabel(labelindex[row - 1]);
        }
        else if (row == 7)
        {
            text = (u8 *)frontModGetOptionLabel(12);
        }
        else if (row == 8)
        {
        ...
        textMeasure(&th, &tw, text, ptrFontZurichBoldChars, ptrFontZurichBold, 0);
        x = 0x39;
        if (mp_player_options_highlight == row && !tab_prev_highlight)
        {
        ...
        {
            text = (u8 *)((flags & MP_PLAYEROPT_HEADROLL) ? "On" : "Off");
        }
        else if (row == 8)
        {
            text = (u8 *)((flags & MP_PLAYEROPT_DAMAGE_SFX) ? "On" : "Off");
        }
        else if (row == 9)
        {
        ...
        {
            opt = optionindex[row - 1];
            if (opt == PLAYER_OPTION_LOOK)
            {
                text = (u8 *)((flags & (1 << (row - 1))) ? "Upright" : "Reverse");
            }
            else if (opt == PLAYER_OPTION_AIM)
            {
                text = (u8 *)((flags & (1 << (row - 1))) ? "Toggle" : "Hold");
            }
            else
            {
                text = (u8 *)((flags & (1 << (row - 1))) ? "On" : "Off");
            }
        }
...
{
#ifdef GE_MODDED_CHEATS
    /* Mod branch: duplicate character selections are intentionally allowed. */
    return FALSE;
#else
    s32 i;
...
#ifdef GE_MODDED_CHEATS
static s32 frontModMpSubmenuBackRequested(MENU target)
{
    s32 i;
...
#ifdef GE_MODDED_CHEATS
    /* R18: Extra MP Characters is a true live roster toggle.  The normal
     * completed-game roster ends at the two Moonraker Elites (33 entries);
     * enabling the cheat exposes all 64 retail table entries, including the
     * developer heads, Terrorist and Biker.  Turning it back off immediately
     * restores the normal 8/33-character progression instead of leaving the
...
void interface_menu0F_mpcharsel(void)
{
    ...
    viSetUseZBuf(0);
#ifdef GE_MODDED_CHEATS
    if (frontModMpSubmenuBackRequested(MENU_MP_OPTIONS))
    {
        return;
    }
#endif
...
#ifdef GE_PHYSICAL_FASTPATHS
/*
 * The multiplayer character carousel renders as many as seven portraits per
 * player every frame.  Retail rebuilt the exact same orthographic/model-view
 * matrices for every portrait.  In the physical fast-path build the caller
 * installs them once per player panel, immediately after the text renderer
 * has finished, and all portraits in that panel share them.
 */
static Gfx *frontSetupCharacterPortraitMatricesFast(Gfx *DL)
{
    Mtx *projection = dynAllocateMatrix();
    Mtx *modelview = dynAllocateMatrix();

    DL = microcode_constructor(DL);
    guOrtho(projection, 0.0f, 440.0f, 0.0f, 330.0f, 1.0f, 10.0f, 1.0f);
    guRotate(modelview, 3.1415927f, 1.0f, 0.0f, 0.0f);
    gSPMatrix(DL++, OS_PHYSICAL_TO_K0(projection), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_PROJECTION);
    gSPMatrix(DL++, OS_PHYSICAL_TO_K0(modelview), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
Gfx *frontRenderCharacterPortrait(Gfx *DL, s32 arg1, s32 arg2, s32 arg3, s32 arg4, s32 arg5, s32 arg6)
{
    ...
    s32 i;
#ifdef GE_PHYSICAL_FASTPATHS
    s32 portrait_alpha[4];

    spCC = dynAllocateVertices(16);

    /* All of these values are invariant across the 16 vertices. */
    sp58 = arg6 + 0x23;
    sp54 = 0x14A - arg4;
    sp50 = arg6 + 0x2a;
    var_v0 = (get_players_who_have_selected_mp_char(arg5) && arg6 == 0) ? 0x6E : 0xFF;
    ...
    if (arg6 == 0)
    {
        ...
        {
            var_s3 = (j >= 2) ? j - 1 : j;
            portrait_alpha[j] = frontCalculateCharacterImageAlpha(((var_s3 - 1) * sp58) + arg3, arg1, arg2);
        }
    }
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            spCC[vtx_index].s = (j & 1) ? 0x1000 : 0;
            spCC[vtx_index].t = (i & 1) ? 0x1000 : 0;
            spCC[vtx_index].r = var_v0;
            spCC[vtx_index].g = var_v0;
            spCC[vtx_index].b = var_v0;
            spCC[vtx_index].a = (arg6 == 0) ? portrait_alpha[j] : 0xFF;
            vtx_index++;
        }
    }
#else
    ...
    simage = &mpcharselimages[selected_photo];
#else
    selected_photo = mp_chr_setup[arg5].select_photo * 4;
...
#ifdef GE_PHYSICAL_FASTPATHS
        DL = frontSetupCharacterPortraitMatricesFast(DL);
#endif
...
void interface_menu10_mphandicap(void)
{
    ...
    viSetUseZBuf(0);
#ifdef GE_MODDED_CHEATS
    if (frontModMpSubmenuBackRequested(MENU_MP_SETTINGS))
    {
        return;
    }
#endif
...
//********************************************************************************************************
#ifdef GE_MODDED_CHEATS
static s32 g_ModMpStagePage = 0;
static s32 g_ModMpStagePageTurnHold = FALSE;
#endif
...
#ifdef GE_MODDED_CHEATS
  g_ModMpStagePage = 0;
  g_ModMpStagePageTurnHold = FALSE;
#endif
  load_walletbond();
}
...
void interface_menu12_mpstage(void)
{
    ...
    s32 stageindex;
#ifdef GE_MODDED_CHEATS
    s32 pagebase = g_ModMpStagePage * 12;
#else
    s32 pagebase = 0;
#endif
    ...
    if (joyGetButtons(PLAYER_1, START_BUTTON|Z_TRIG|A_BUTTON) == 0)
    {
        ...
        {
            /* R5: after turning to page 2, leave the cursor parked over the
             * page-turn tab until the player actually moves it away.  Without
             * this latch the same cursor coordinates are immediately decoded
             * as a stage cell and Statue highlights itself on the next frame. */
            if (g_ModMpStagePage == 0)
            {
                tab_next_highlight = TRUE;
            }
        }
        ...
        else
        {
            ...
            g_ModMpStagePageTurnHold = FALSE;
#endif
            if ((s32)cursor_v_pos >= 240) row = 2;
            else if ((s32)cursor_v_pos >= 170) row = 1;
            else row = 0;

            if ((s32)cursor_h_pos >= 292) column = 3;
            else if ((s32)cursor_h_pos >= 207) column = 2;
            else if ((s32)cursor_h_pos >= 122) column = 1;
            else column = 0;

            localindex = row * 4 + column;
            stageindex = pagebase + localindex;
            if (stageindex >= MP_STAGE_SELECTED_MAX) stageindex = MP_STAGE_SELECTED_MAX - 1;

            while (stageindex >= pagebase && check_if_mp_stage_unlocked(stageindex) == 0)
            {
                stageindex--;
            }
            if (stageindex >= pagebase)
            {
                current_mp_stage_highlighted = stageindex;
            }
        }
    }
    if (joyGetButtonsPressedThisFrame(PLAYER_1, START_BUTTON|Z_TRIG|A_BUTTON))
...
#ifdef GE_MODDED_CHEATS
        else if (tab_next_highlight)
        {
            g_ModMpStagePage = 1;
            g_ModMpStagePageTurnHold = TRUE;
            current_mp_stage_highlighted = -1;
        }
#endif
        else if (current_mp_stage_highlighted >= MP_STAGE_RANDOM && check_if_mp_stage_unlocked(current_mp_stage_highlighted))
        {
            tab_next_selected = TRUE;
...
Gfx * constructor_menu12_mpstage(Gfx *DL)
{
    ...
    for (spB4 = 0; spB4 != 3; spB4++)
    {
        ...
        for (i_3 = 0; i_3 < 4; i_3++)
        {
            stageindex = pagebase + count;
            if (count < 12 && stageindex < MP_STAGE_SELECTED_MAX)
            {
                simage = &mpstageselimages[multi_stage_setups[stageindex].photo];

                texSelect(&DL, simage, 1, 0, 2);

                if (stageindex == current_mp_stage_highlighted)
                {
    ...
    if(1)
    {
        ...
        for (sp9C = 0; sp9C != 3; sp9C++)
        {
            ...
            for (i_4 = 0; i_4 < 4; i_4++)
            {
                stageindex = pagebase + count_2;
                if (count_2 < 12 && stageindex < MP_STAGE_SELECTED_MAX && check_if_mp_stage_unlocked(stageindex))
                {
                    var_s2_2 = 0x96969600;
                    ...
                    textMeasure(&sp128, &sp124, langGet(multi_stage_setups[stageindex].select_screen_text_preset), ptrFontBankGothicChars, ptrFontBankGothic, 0);
                    ...
                    DL = textRender(DL, &sp130, &sp12C, langGet(multi_stage_setups[stageindex].select_screen_text_preset), ptrFontBankGothicChars, ptrFontBankGothic, var_s2_2 | 0xFF, viGetX(), viGetY(), 0, 0);

                    sp130 = var_s6 - 0x1F;
                    sp12C = sp90 - sp128;

                    DL = textRender(DL, &sp130, &sp12C, langGet(multi_stage_setups[stageindex].select_screen_text_preset), ptrFontBankGothicChars, ptrFontBankGothic, var_s2_2 | 0x64, viGetX(), viGetY(), 0, 0);
                }
...
void interface_menu13_mpscenario(void)
{
    ...
    if (joyGetButtons(PLAYER_1, Z_TRIG|A_BUTTON) == 0) {
        ...
        else
        {
            ...
            for (i = MPSCENARIOS_MAX - 1; i != 0; i--)
            {
#ifdef GE_MODDED_CHEATS
                scenarioid = frontMpScenarioForDisplaySlot(i);
                if ((s32)cursor_v_pos >= (0x7f + (i * 0x16))
                    && frontMpScenarioAllowedForCount(scenarioid, get_selected_num_players()))
                {
                    dword_CODE_bss_80069780 = scenarioid + 1;
                    break;
                }
#else
                if ((s32)cursor_v_pos >= (0x83 + (i * 0x16))
...
Gfx * constructor_menu13_mpscenario(Gfx *DL)
{
    ...
    for (i = 0; i < MPSCENARIOS_MAX; i++)
    {
        var_s3 = 0xff;
#ifdef GE_MODDED_CHEATS
        sp7C = frontMpScenarioForDisplaySlot(i);
#endif

        if (
#ifdef GE_MODDED_CHEATS
            !frontMpScenarioAllowedForCount(sp7C, get_selected_num_players())
#else
            ((s32) mp_player_counts[i].max < get_selected_num_players()) || (get_selected_num_players() < (s32) mp_player_counts[i].min)
#endif
        )
        {
            var_s3 = 0x70;
...
#ifdef GE_MODDED_CHEATS
        text = frontGetMpScenarioText(sp7C);
#else
...
#ifdef GE_MODDED_CHEATS
        y = 0x7f + (i * 0x16);
        if ((frontMpScenarioForDisplaySlot(i) + 1) == dword_CODE_bss_80069780)
#else
        y = 0x83 + (i * 0x16);
        ...
        if (ptrbriefingdata->objective[i].textid != 0 && selected_difficulty >= ptrbriefingdata->objective[i].enabled_difficulty)
        {
            ...
            *arg2 = *arg2 + sp98;
            ...
            if (arg3)
            {
                switch (
#ifdef GE_MODDED_CHEATS
                    frontGetMissionReportObjectiveStatus(i)
#else
...
s32 frontCompleteAllObjectivesAliveSuccess(void)
{
    ...
    {
        return g_CoopMissionReport.success;
    }
#endif
...
Gfx * constructor_menu0C_missionfailed(Gfx *DL)
{
    ...
    if (
#ifdef GE_MODDED_CHEATS
        frontHasCoopMissionReport() ? g_CoopMissionReport.bondkia :
#endif
        g_isBondKIA)
    {
...
#ifdef GE_MODDED_CHEATS
        frontHasCoopMissionReport() ? g_CoopMissionReport.missionfailed :
#endif
...
void interface_menu0D_missioncomplete()
{
    ...
    if (tab_next_selected)
    {
        if (frontCompleteAllObjectivesAliveSuccess() && (!
#ifdef GE_MODDED_CHEATS
            (frontHasCoopMissionReport() ? g_CoopMissionReport.cheatsused : g_AppendCheatSinglePlayer)
#else
            g_AppendCheatSinglePlayer
#endif
        ))
        {
...
#ifdef GE_MODDED_CHEATS
static Gfx *frontCoopStatsPrintCentered(Gfx *DL, const char *text, s32 centerx, s32 y, u32 colour)
{
    s32 textheight = 0;
    s32 textwidth = 0;
    s32 x;

    textMeasure(&textheight, &textwidth, (char *)text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
    x = centerx - (textwidth >> 1);
    return frontPrintText(DL, &x, &y, (char *)text, ptrFontBankGothicChars, ptrFontBankGothic,
            colour, viGetX(), viGetY(), 0, 0);
}

static void frontCoopStatsFormatWeapon(char *dst, s32 player, s32 maxwidth)
{
    s32 hand0 = g_CoopMissionReport.favoriteweapon[player][0];
    s32 hand1 = g_CoopMissionReport.favoriteweapon[player][1];
    char *src;
    s32 len;
    s32 textheight;
    s32 textwidth;
    ...
    src = (char *)get_ptr_short_watch_text_for_item(hand0);
    strcpy(dst, src ? src : "-");
    ...
    if (hand1 == hand0 && hand0 > 0 && len < 27)
    {
        strcat(dst, " x2");
    }

    /* Keep each player on one line.  Only unusually long localized weapon
     * names are shortened, and only enough to fit the weapon column. */
    for (;;)
    {
        textheight = 0;
        textwidth = 0;
        textMeasure(&textheight, &textwidth, dst, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        if (textwidth <= maxwidth || strlen(dst) <= 3)
        {
            break;
        }
        len = strlen(dst);
...
static Gfx *constructor_menu0D_coopmissionstats(Gfx *DL)
{
...

    {
        s32 x = 0x37;
        s32 titley = 0x8F;
        DL = frontPrintText(DL, &x, &titley, "CO-OP STATISTICS:",
                ptrFontZurichBoldChars, ptrFontZurichBold, 0xFF, viGetX(), viGetY(), 0, 0);
    }
    ...
    for (p = 0; p < g_CoopMissionReport.playercount; p++)
    {
        ...
        hitshots = g_CoopMissionReport.shotcount[p][1]
            + g_CoopMissionReport.shotcount[p][2]
            + g_CoopMissionReport.shotcount[p][3]
            + g_CoopMissionReport.shotcount[p][4]
...
Gfx *constructor_menu0D_missioncomplete(Gfx *DL)
{
    ...
    if (frontHasCoopMissionReport())
    {
        return constructor_menu0D_coopmissionstats(DL);
    }
#endif
...
#ifdef GE_MODDED_CHEATS
/* Hidden gameplay cheat entries which retail never places in Cheat Options.
 * Their original text-bank IDs are intentionally left untouched; entries
 * without a retail menu label therefore render as the preserved "NO NAME". */
static s32 frontModIsHiddenCheat(s32 cheat)
{
    switch (cheat)
    {
        case CHEAT_EXTRA_MP_CHARS:
...
u8 *frontModGetCheatMenuText(s32 cheat)
{
    switch (cheat)
    {
        ...
        case CHEAT_2X_HEALTH: return (u8 *)"2x Health";
        case CHEAT_2X_ARMOR: return (u8 *)"2x Armor";
        case CHEAT_EXTRA_WEAPONS: return (u8 *)"Extra Weapons";
        case CHEAT_10X_HEALTH: return (u8 *)"10x Health";
        case CHEAT_BONDPHASE: return (u8 *)"Invisible [Multi]";
...
static void frontModBuildCheatPage(s32 page)
{
    ...
    totalunlockedcheats = 0;
    cheathighlighted = 0;
    tab_start_selected = page; /* unused by Cheat Options; used as page index here */

    if (page == 0)
    {
        for (i = 1; i != CHEAT_INVALID; i++)
        {
            /* R17: Line Mode is always exposed on Page 1 at its natural ID
             * position, directly after All Guns and before Bond Invisible. */
            if (i == CHEAT_LINEMODE)
            {
                arrayUnlockedCheats[totalunlockedcheats++] = CHEAT_LINEMODE;
            }
            else if (cheat_available[i] && !frontModIsHiddenCheat(i))
            {
                arrayUnlockedCheats[totalunlockedcheats++] = i;
            }

            /* No Reload stays visible on page 1 directly under Infinite Ammo;
             * sits directly below Infinite Ammo on Page 1. */
            if (i == CHEAT_INFINITE_AMMO)
            {
                arrayUnlockedCheats[totalunlockedcheats++] = CHEAT_NO_RELOAD;
            }
        }
    }
    else
    {
        for (i = 0; i < (s32)(sizeof(hiddencheats) / sizeof(hiddencheats[0])); i++)
        {
            arrayUnlockedCheats[totalunlockedcheats++] = hiddencheats[i];
        }
    }
}
#endif
...
#ifdef GE_MODDED_CHEATS
        else if ((tab_start_selected == 0) && frontCheckCursorOnNextTab())
        {
            tab_next_highlight = 1;
        }
#endif
...
#ifdef GE_MODDED_CHEATS
        else if (tab_next_highlight != 0)
        {
            frontModBuildCheatPage(1);
        }
#endif
        else
        {
...
Gfx * constructor_menu15_cheat(Gfx *DL)
{
    ...
    {
        #ifdef GE_MODDED_CHEATS
        temp_v0 = frontModGetCheatMenuText(arrayUnlockedCheats[var_fp]);
#else
    ...
    if (totalunlockedcheats >= 0xd)
    {
        ...
        {
            #ifdef GE_MODDED_CHEATS
            temp_v0 = frontModGetCheatMenuText(arrayUnlockedCheats[var_fp + 12]);
#else
...
#ifdef GE_MODDED_CHEATS
//********************************************************************************************************
//MODDED FRONTEND OPTIONS
// Save-backed frontend settings. R21 page 2 carries death/damage presentation toggles.
//********************************************************************************************************
static s32 g_ModOptionsHighlighted;
static s32 g_ModOptionsDirty;
static s32 g_ModOptionsPage;
...
#define MOD_OPTIONS_HIGHLIGHT_X2  387

void init_menu_mod_options(void)
{
    save_data *save = fileGetSaveForFoldernum(selected_folder_num);
    ...
    {
        save->options |= OPTION_HEADROLL | OPTION_R21_MIGRATED;
    ...
    if (save && (save->mod_options3 & MODOPT3_SIGNATURE_MASK) != MODOPT3_SIGNATURE)
    {
        ...
        {
            save->options &= ~OPTION_INVERTLOOK;
...
void update_menu_mod_options(void) { }

static void frontModOptionsChange(s32 direction)
{
    save_data *save = fileGetSaveForFoldernum(selected_folder_num);
    u16 opt;
    s32 value;
    ...
    switch (g_ModOptionsHighlighted)
    {
        case 0:
            value = (s32)save->music_vol + direction * 8;
            if (value < 0) value = 0; if (value > 255) value = 255;
            save->music_vol = value;
            frontModApplyMusicVolume(save);
            break;
        case 1:
            value = (s32)save->sfx_vol + direction * 8;
            if (value < 0) value = 0; if (value > 255) value = 255;
            save->sfx_vol = value;
            frontModApplySfxVolume(save);
            break;
        case 2:
            value = (opt & OPTION_CONTROLTYPE) >> 8;
            value = (value + direction + 8) % 8;
            save->options = (opt & ~OPTION_CONTROLTYPE) | ((value << 8) & OPTION_CONTROLTYPE);
            break;
        case 3:
            save->options = opt ^ OPTION_INVERTLOOK;
            /* Keep the live Watch option state in lockstep with the folder bit.
             * Retail semantics are: clear = Reverse, set = Upright/Up-Down. */
            set_cur_player_look_vertical_inverted((save->options & OPTION_INVERTLOOK) != 0);
            break;
        case 4: save->options = opt ^ OPTION_AUTOAIM; break;
        case 5: save->options = opt ^ OPTION_AIMCONTROL; break;
        case 6: save->options = opt ^ OPTION_SIGHTONSCREEN; break;
        case 7: save->options = opt ^ OPTION_LOOKAHEAD; break;
        case 8: save->options = opt ^ OPTION_DISPLAYAMMO; break;
        case 9:
            value = (opt & OPTION_SCREENCINEMA) ? 2 : ((opt & OPTION_SCREENWIDE) ? 1 : 0);
            value = (value + direction + 3) % 3;
            opt &= ~(OPTION_SCREENWIDE | OPTION_SCREENCINEMA);
            if (value == 1) opt |= OPTION_SCREENWIDE;
            if (value == 2) opt |= OPTION_SCREENCINEMA;
            save->options = opt;
            break;
        case 10:
            value = ((opt & OPTION_SCREENRATIO) ? 1 : 0) | ((opt & OPTION_SCREENRATIO2) ? 2 : 0);
            value = (value + direction + 4) % 4;
            opt &= ~(OPTION_SCREENRATIO | OPTION_SCREENRATIO2);
            if (value & 1) opt |= OPTION_SCREENRATIO;
            if (value & 2) opt |= OPTION_SCREENRATIO2;
            save->options = opt | OPTION_R21_MIGRATED;
            break;
    }
    g_ModOptionsDirty = TRUE;
}
...
void interface_menu_mod_options(void)
{
    ...
    if (g_ModOptionsUnlockConfirm)
    {
        ...
        if (cursor_v_pos >= 136.0f && cursor_v_pos <= 164.0f)
            g_ModOptionsUnlockChoice = cursor_h_pos >= 220.0f;
        if (pressed & (L_JPAD | L_CBUTTONS)) g_ModOptionsUnlockChoice = 0;
        if (pressed & (R_JPAD | R_CBUTTONS)) g_ModOptionsUnlockChoice = 1;
        ...
        {
            sndPlaySfx(g_musicSfxBufferPtr, DOOR_METAL_CLOSE2_SFX, NULL);
            if (g_ModOptionsUnlockChoice)
            {
                fileUnlockEverythingInFolder(selected_folder_num);
            }
            g_ModOptionsUnlockConfirm = FALSE;
            g_ModOptionsUnlockChoice = 0;
        }
    ...
    if ((pressed & B_BUTTON) || (tab_prev_highlight && (pressed & (START_BUTTON|Z_TRIG|A_BUTTON))))
    {
        if (g_ModOptionsPage)
        {
            g_ModOptionsPage = 0;
        ...
        else
        {
            ...
            frontChangeMenu(MENU_MODE_SELECT, FALSE);
            setCursorPOSforMode(3);
        }
        sndPlaySfx(g_musicSfxBufferPtr, DOOR_METAL_CLOSE2_SFX, NULL);
        return;
    }
    if (tab_next_highlight && (pressed & (START_BUTTON|Z_TRIG|A_BUTTON)))
    {
        g_ModOptionsPage = 1;
        g_ModOptionsHighlighted = 0;
        sndPlaySfx(g_musicSfxBufferPtr, DOOR_METAL_CLOSE2_SFX, NULL);
    ...
    if (!tab_prev_highlight && !tab_next_highlight)
    {
        ...
        if (g_ModOptionsUnlockHover && (pressed & (START_BUTTON|Z_TRIG|A_BUTTON)))
        {
            g_ModOptionsUnlockConfirm = TRUE;
            g_ModOptionsUnlockChoice = 0;
            sndPlaySfx(g_musicSfxBufferPtr, DOOR_METAL_CLOSE2_SFX, NULL);
            disable_all_switches(walletinst[0]);
            set_item_visibility_in_objinstance(walletinst[0], SW_TABS, 1);
        ...
        if (!g_ModOptionsUnlockHover && !g_ModOptionsPage && (row == 0 || row == 1) && joyGetButtons(PLAYER_1, Z_TRIG|A_BUTTON))
        {
            ...
            f32 t = (cursor_h_pos - (f32)MOD_OPTIONS_SLIDER_X1) /
                    (f32)(MOD_OPTIONS_SLIDER_X2 - MOD_OPTIONS_SLIDER_X1);
            s32 value;

            if (t < 0.0f) t = 0.0f;
            if (t > 1.0f) t = 1.0f;
            value = (s32)(t * 255.0f + 0.5f);

            if (save)
            {
                if (row == 0 && save->music_vol != value)
                {
                    save->music_vol = value;
                    frontModApplyMusicVolume(save);
                    g_ModOptionsDirty = TRUE;
                }
                else if (row == 1 && save->sfx_vol != value)
                {
                    save->sfx_vol = value;
                    frontModApplySfxVolume(save);
                    g_ModOptionsDirty = TRUE;
    ...
    disable_all_switches(walletinst[0]); set_item_visibility_in_objinstance(walletinst[0], SW_TABS, 1); set_item_visibility_in_objinstance(walletinst[0], SW_BLANK, 1);
    frontUpdateControlStickPosition();
}
...
Gfx *constructor_menu_mod_options(Gfx *DL)
{
    ...
    if (g_ModOptionsUnlockConfirm)
    {
        ...
        /* frontModGetOptionLabel uses one scratch buffer, so consume each
         * label before asking for the next one. */
        text = frontModGetOptionLabel(18);
        textMeasure(&th, &tw, text, ptrFontBankGothicChars, ptrFontBankGothic, 0);
        x = 220 - (tw >> 1); y = 67;
        DL = frontPrintText(DL,&x,&y,text,ptrFontBankGothicChars,ptrFontBankGothic,0xFF,viGetX(),viGetY(),0,0);

        text = frontModGetOptionLabel(19);
        textMeasure(&th, &tw, text, ptrFontZurichBoldChars, ptrFontZurichBold, 0);
        x = 220 - (tw >> 1); y = 108;
        DL = frontPrintText(DL,&x,&y,text,ptrFontZurichBoldChars,ptrFontZurichBold,0xFF,viGetX(),viGetY(),0,0);

        if (g_ModOptionsUnlockChoice == 0)
            DL=microcode_constructor_related_to_menus(DL,128,139,184,159,0x32);
        else
            DL=microcode_constructor_related_to_menus(DL,256,139,312,159,0x32);

        text = frontModGetOptionLabel(20);
        textMeasure(&th, &tw, text, ptrFontZurichBoldChars, ptrFontZurichBold, 0);
        x=156-(tw>>1); y=142; DL=frontPrintText(DL,&x,&y,text,ptrFontZurichBoldChars,ptrFontZurichBold,0xFF,viGetX(),viGetY(),0,0);
        text += 3;
        textMeasure(&th, &tw, text, ptrFontZurichBoldChars, ptrFontZurichBold, 0);
        x=284-(tw>>1); y=142; DL=frontPrintText(DL,&x,&y,text,ptrFontZurichBoldChars,ptrFontZurichBold,0xFF,viGetX(),viGetY(),0,0);
        DL=frontDrawCursor(DL);
        return DL;
    }
    ...
    for (i=0;i<count;i++)
    {
        ...
        if (i==g_ModOptionsHighlighted && !g_ModOptionsUnlockHover && !tab_prev_highlight && !tab_next_highlight)
        {
            ...
            DL=microcode_constructor_related_to_menus(DL,53,y-1,57+lw,y+14,0x32);
        }
        DL=frontPrintText(DL,&x,&y,(u8*)label,ptrFontZurichBoldChars,ptrFontZurichBold,0xFF,viGetX(),viGetY(),0,0);

        if (!g_ModOptionsPage && (i == 0 || i == 1))
        {
            value = i == 0 ? (save ? save->music_vol : 255) : (save ? save->sfx_vol : 255);
            sliderfill = MOD_OPTIONS_SLIDER_X1 + ((MOD_OPTIONS_SLIDER_X2 - MOD_OPTIONS_SLIDER_X1) * value) / 255;
            DL=microcode_constructor_related_to_menus(DL,MOD_OPTIONS_SLIDER_X1,y+5,MOD_OPTIONS_SLIDER_X2,y+10,0x32);
            if (sliderfill > MOD_OPTIONS_SLIDER_X1) DL=microcode_constructor_related_to_menus(DL,MOD_OPTIONS_SLIDER_X1,y+5,sliderfill,y+10,0x64);
            percent = (value * 100 + 127) / 255;
            sprintf(percenttext, "%d%%", percent);
            x=MOD_OPTIONS_PERCENT_X;
            DL=frontPrintText(DL,&x,&y,(u8*)percenttext,ptrFontZurichBoldChars,ptrFontZurichBold,0xFF,viGetX(),viGetY(),0,0);
            continue;
        }
        ...
        else if (i==3) valueptr=(opt&OPTION_INVERTLOOK)?"Up/Down":"Reverse";
        else if (i==4) valueptr=(opt&OPTION_AUTOAIM)?"On":"Off";
        else if (i==5) valueptr=(opt&OPTION_AIMCONTROL)?"Toggle":"Hold";
        else if (i==6) valueptr=(opt&OPTION_SIGHTONSCREEN)?"On":"Off";
        else if (i==7) valueptr=(opt&OPTION_LOOKAHEAD)?"On":"Off";
        else if (i==8) valueptr=(opt&OPTION_DISPLAYAMMO)?"On":"Off";
        else if (i==9) valueptr=(opt&OPTION_SCREENCINEMA)?"Cinema":((opt&OPTION_SCREENWIDE)?"Wide":"Full");
        else if (i==10) {
            value=((opt&OPTION_SCREENRATIO)?1:0)|((opt&OPTION_SCREENRATIO2)?2:0);
            valueptr=value==1?"16:9":(value==2?"21:9":(value==3?"32:9":"4:3"));
        }
        x=202; DL=frontPrintText(DL,&x,&y,(u8*)valueptr,ptrFontZurichBoldChars,ptrFontZurichBold,0xFF,viGetX(),viGetY(),0,0);
    }
    DL=frontAddPreviousTabText(DL);
