...
#ifdef GE_MODDED_CHEATS
    /*
     * Stage/global AI lists still contain retail opcodes whose "Bond" means
     * g_CurrentPlayer.  They are not guard-local and therefore have no Co-Op
     * target sidecar.  Run them against a deterministic primary player rather
     * than whichever split-screen viewport happened to render last.  Local
