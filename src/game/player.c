...
#ifdef GE_MODDED_CHEATS
/*
 * R18: GoldenEye's tank relationship was global because retail campaign only
 * had one Bond.  Campaign Co-Op needs the four pieces which describe Bond's
 * relationship to the tank to follow the current player instead.  Perfect
 * Dark moved equivalent vehicle/tank state into struct player; preserve GE's
 * ABI here with a tiny sidecar indexed by player number.
 */
struct CoopTankPlayerState
{
    s32 isInTank;
...
static void playerSaveCoopTankContext(s32 index)
{
    if (index < 0 || index >= MAX_PLAYER_COUNT || !playerIsCampaignCoop())
    {
        return;
    }

    g_CoopTankPlayerState[index].isInTank = g_PlayerIsInTank;
    g_CoopTankPlayerState[index].tankProp = g_PlayerTankProp;
    g_CoopTankPlayerState[index].yOffset = g_PlayerTankYOffset;
    g_CoopTankPlayerState[index].canEnter = g_BondCanEnterTank;
    g_CoopTankPlayerState[index].explodeOnDeath = g_ExplodeTankOnDeathFlag;
    g_CoopTankPlayerState[index].damagePenaltyTicks = g_TankDamagePenaltyTicks;
}

static void playerLoadCoopTankContext(s32 index)
{
    if (index < 0 || index >= MAX_PLAYER_COUNT || !playerIsCampaignCoop())
    {
        return;
    ...
    g_PlayerIsInTank = g_CoopTankPlayerState[index].isInTank;
    g_PlayerTankProp = g_CoopTankPlayerState[index].tankProp;
    g_PlayerTankYOffset = g_CoopTankPlayerState[index].yOffset;
    g_BondCanEnterTank = g_CoopTankPlayerState[index].canEnter;
    g_ExplodeTankOnDeathFlag = g_CoopTankPlayerState[index].explodeOnDeath;
    g_TankDamagePenaltyTicks = g_CoopTankPlayerState[index].damagePenaltyTicks;
}

static void playerResetCoopTankContext(s32 index)
{
    ...
    g_CoopTankPlayerState[index].isInTank = FALSE;
    g_CoopTankPlayerState[index].tankProp = NULL;
    g_CoopTankPlayerState[index].yOffset = 0.0f;
...
{
    if (!playerIsCampaignCoop())
    {
        return TRUE;
    }

    return g_CoopTankOwnerPlayer < 0 || g_CoopTankOwnerPlayer == player_num;
}
...
void playerCoopTankClaimCurrentPlayer(void)
{
    if (playerIsCampaignCoop())
    {
        g_CoopTankOwnerPlayer = player_num;
    }
}

void playerCoopTankReleaseCurrentPlayer(void)
{
    if (playerIsCampaignCoop() && g_CoopTankOwnerPlayer == player_num)
    {
...
#ifdef GE_MODDED_CHEATS
    playerResetCoopTankContext(PLAYER_1);
    playerResetCoopTankContext(PLAYER_2);
...
#ifdef GE_MODDED_CHEATS
    /* Save the outgoing Bond's tank/climb relationship before changing the
     * global current-player pointer, then restore the incoming Bond's copy. */
    if (g_CurrentPlayer != NULL)
    {
        playerSaveCoopTankContext(player_num);
    }
#endif
...
#ifdef GE_MODDED_CHEATS
    playerLoadCoopTankContext(playernum);
#endif
}
...
PROP getPropForHeldItem(ITEM_IDS arg0)
{
    ...
    switch (arg0)
    {
        ...
        case ITEM_SILVERWPPK:    ret = PROP_CHRSILVERWPPK;    break; /* weapon_multi_pp7_special1 */
        case ITEM_GOLDWPPK:      ret = PROP_CHRGOLDWPPK;      break; /* weapon_multi_pp7_special2 */
