 ...
 * GoldenEye normally waits for its age-based unload tick; under memory
 * pressure that can leave an adjacent room absent until crossing a portal.
 * Reclaim the stalest non-visible, non-neighbour, non-script-pinned room
 * until the requested cached room block can fit contiguously.
 */
void bgModGarbageCollectRoomsForLoad(s32 bytesneeded)
{
    ...
    while (memaGetLongestFree() < bytesneeded)
    {
        oldestroom = 0;
        oldestage = 1;
        ...
        delete_room_data(oldestroom);
        /* Match Perfect Dark's room collector: coalesce adjacent free blocks
         * immediately so a sufficiently large total free pool becomes a
         * usable contiguous room allocation on the same load attempt. */
        memaDefrag();
    }
}
...
void fileUnlockEverythingInFolder(s32 foldernum)
{
    ...
    if (foldernum < FOLDER1 || foldernum >= MAX_FOLDER_COUNT) return;
    save = fileGetSaveForFoldernum(foldernum);
    if (!save) return;

    /* Exact former Everything Unlocked save state, scoped to one folder. */
    for (i = 0; i < (s32)sizeof(save->times); i++) save->times[i] = 0xff;
    save->flag_007 |= 1;
...
/*
 * R21: Keep the mod Options labels out of the compressed C data segment.
