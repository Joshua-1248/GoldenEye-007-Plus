...
//CODE.bss:800799A4
s32 dword_CODE_bss_800799A4; // RC5 reuses this unused retail word as Real-Time Collapse phase
#define g_Rc5RealtimeCollapsePhase dword_CODE_bss_800799A4
...
//CODE.bss:800799CC
f32 flt_CODE_bss_800799CC; // RC5 reuses this unused retail word as Real-Time Collapse timer
#define g_Rc5RealtimeCollapseTimer flt_CODE_bss_800799CC
...
#ifdef GE_MODDED_CHEATS
        || (lvlIsCoopEndCutscene() && get_cur_playernum() == 0)
#endif
    ))
    {
...
void bondviewSetCameraMode(s32 arg0)
{
     ...
     * initialization uses g_CurrentPlayer. Always initialize the shared
     * cinematic on P1, then restore the caller's simulation owner. */
    if (arg0 == CAMERAMODE_POSEND
        && gamemode == GAMEMODE_MULTI && get_scenario() == SCENARIO_COOP
        && get_cur_playernum() != PLAYER_1)
    {
        coop_restore_player = get_cur_playernum();
    ...
    else if (g_CameraMode == CAMERAMODE_DEATH_CAM_SP)
    {
        ...
        else
        {
            ...
            g_PlayerIsInTank = 0;
#ifdef GE_MODDED_CHEATS
            /* A Co-Op driver dying must release the shared driver's seat so
             * another surviving player can claim the tank. */
...
#ifdef GE_MODDED_CHEATS
            /* Endless Death Cam must not inherit retail's finite/failure exit.
             * Keep the last valid camera position and let the replay animation
             * finish; the normal SP->MP fade cycle will then start another
             * replay. Explicit A/B/Z/START exit still sets camera_mode to
             * FADESWIRL and is handled by the normal path. */
...
#ifdef GE_MODDED_CHEATS
    if (coop_restore_player >= 0)
    {
        set_cur_player(coop_restore_player);
    }
#endif
}
...
void bondviewAdvanceCameraMode(void)
{
    ...
    else if (mode != CAMERAMODE_MP)
    {
        ...
        else if (mode != CAMERAMODE_FP)
        {
            ...
            {
#ifdef GE_MODDED_CHEATS
                /* RC5: Endless Death Cam repeats only the visual replay.
                 * camera_mode == 0 is also the one-shot death-music gate when
                 * entering CAMERAMODE_DEATH_CAM_SP, so advance it once after
                 * the first replay and keep it there for endless visual loops.
                 * The retail exit input sets camera_mode to FADESWIRL; do not
                 * intercept that value, so A/B/Z/START still exits normally. */
                if (gamemode != GAMEMODE_MULTI
                    && (g_ModGameplayOptions2 & MODOPT2_ENDLESS_DEATHCAM)
                    && camera_mode != CAMERAMODE_FADESWIRL)
                {
                    if (camera_mode == CAMERAMODE_NONE)
                    {
                ...
                {
                    camera_mode++;
                    if (camera_mode < CAMERAMODE_SWIRL) bondviewSetCameraMode(CAMERAMODE_DEATH_CAM_SP);
                }
            }
        }
    }
...
void bondviewCalcUpdatePlayerCollision(struct coord3d *offset, s32 allow_scoot)
{
    ...
    if (g_WorldTankProp != NULL)
    {
        ...
        else
        {
            ...
            {
#ifdef GE_MODDED_CHEATS
                /* R18 Co-Op: g_WorldTankProp describes the shared world tank,
                 * while g_PlayerTankProp is now player-local via the sidecar.
                 * One player stepping away must not erase the shared tank
                 * discovery used by another player who is standing on/driving it. */
...
void bondviewMoveAnimationTick(f32 speed, f32 speedforwards, f32 speedsideways)
{
    ...
    else
    {
        if (g_CurrentPlayer->startnewbonddie)
        {
            ...
            {
                /* Retail immediately runs this animation at model speed 0.25
                 * (bheadSetSpeed(0.5)). Keep that exact wall-clock budget even
                 * though the option renders it at model speed 0.5. */
                g_Rc5RealtimeCollapseTimer = modelGetAnimEndFrame(&g_CurrentPlayer->model) * 4.0f;
                g_Rc5RealtimeCollapsePhase = 1;
            }
#endif
            g_CurrentPlayer->startnewbonddie = FALSE;
        }

        bheadSetSpeed((gamemode != GAMEMODE_MULTI && (g_ModGameplayOptions2 & MODOPT2_REALTIME_COLLAPSE)) ? 1.0f : 0.5f);
        speedsideways = 0.0f;
    }
    ...
    if (cur_player_get_headroll_setting())
    {
...
void bondviewProcessInput(s8 stick_x, s8 stick_y, u16 buttons, u16 oldbuttons)
{
    ...
    if (moveData.btap)
    {
        ...
        /* If Bond is standing on the tank and pressed B, enter the tank. */
            ...
            && g_BondCanEnterTank
#ifdef GE_MODDED_CHEATS
            && playerCoopTankCanCurrentPlayerClaim()
#endif
            )
        {
            ...
            g_PlayerIsInTank = 1;
#ifdef GE_MODDED_CHEATS
            playerCoopTankClaimCurrentPlayer();
#endif
    ...
    if (bondviewGetIfCurrentPlayerDamageShowTime() && getPlayerCount() == 1 && !(g_ModGameplayOptions2 & MODOPT2_DISABLE_HITSTUN))
    {
        moveData.triggerOn = 0;
    }
    ...
    gunSetSightVisible(GUNSIGHTREASON_NOTAIMING,
        moveData.aiming || cur_player_get_crosshair_setting());
...
// Ensure we don't read out of bounds of the g_DamageTypes array.
                ...
                || (g_DamageTypes[g_CurrentPlayer->damagetype].flashEndFrame >= g_CurrentPlayer->damageshowtime)))
        {
                ...
                && g_CurrentPlayer->damageshowtime <= g_DamageTypes[g_CurrentPlayer->damagetype].flashEndFrame)
            {
                ...
                if (g_ModGameplayOptions2 & MODOPT2_DAMAGE_FLASH)
                {
...
#ifdef GE_MODDED_CHEATS
        /* R21 RC9: keep the scripted hazard alive through the death sequence,
         * then stop creating new permanent explosions once terminal GAME OVER
         * owns the world.  mpStartCoopGameOver also pauses the simulation. */
...
{
#ifdef GE_MODDED_CHEATS
    /* During the shared Co-Op exit cinema only P1 adopts the solo viewport.
     * Hidden P2-P4 ticks retain their normal split-screen geometry. */
...
void bondviewMovePlayerUpdateViewport(s8 stick_x, s8 stick_y, u16 buttons)
{
    ...
    {
        faspect = ((f32) bondviewGetCurrentPlayerViewportWidth() / (f32) bondviewGetCurrentPlayerViewportHeight()) * get_screen_ratio_factor();
    }
    else
    {
    ...
    if (get_screen_ratio() != SCREEN_RATIO_NORMAL)
    {
        set_cur_player_aspect(((f32) bondviewGetCurrentPlayerViewportWidth() / (f32) bondviewGetCurrentPlayerViewportHeight()) * get_screen_ratio_factor());
        viSetAspect(((f32) bondviewGetCurrentPlayerViewportWidth() / (f32) bondviewGetCurrentPlayerViewportHeight()) * get_screen_ratio_factor());
    }
    else
    {
    ...
    gunSetSightVisible(
        GUNSIGHTREASON_1,
        cur_player_get_crosshair_setting()
...
Gfx *maybe_mp_interface(Gfx *gdl)
{
...
#ifdef GE_MODDED_CHEATS
        if (getPlayerCount() == 1 || get_scenario() != SCENARIO_COOP)
#endif
    ...
    if (g_CurrentPlayer->bonddead != 0)
    {
        ...
        if (!g_CurrentPlayer->deathanimfinished
            && gamemode != GAMEMODE_MULTI
            && (g_ModGameplayOptions2 & MODOPT2_REALTIME_COLLAPSE)
            && g_Rc5RealtimeCollapsePhase != 0
            && g_Rc5RealtimeCollapseTimer > 0.0f)
        {
            g_Rc5RealtimeCollapseTimer -= g_GlobalTimerDelta;
        }
#endif
            ...
            modelGetAnimEndFrame(&g_CurrentPlayer->model))
        {
            if (g_CurrentPlayer->redbloodfinished)
            {
                if (!g_CurrentPlayer->deathanimfinished
                    ...
                    && (gamemode == GAMEMODE_MULTI
                        || !(g_ModGameplayOptions2 & MODOPT2_REALTIME_COLLAPSE)
                        || (g_Rc5RealtimeCollapsePhase != 0 && g_Rc5RealtimeCollapseTimer <= 0.0f))
#endif
                )
                {
                ...
                /* Real-Time Collapse can deliberately sit on the final
                 * head-animation frame before deathanimfinished is committed.
                 * Do not let an already-complete gameplay fade bypass that
                 * hold and jump straight into the death camera. Retail reaches
                 * this point only after deathanimfinished becomes true. */
                if (g_CurrentPlayer->deathanimfinished && currentPlayerIsFadeComplete())
                {
                    if (getPlayerCount() == 1)
                    {
                        bondviewSetCameraMode(CAMERAMODE_DEATH_CAM_SP);
    ...
    gdl = sub_GAME_7F08AAE8(gdl);
#ifdef GE_MODDED_CHEATS
    /* RC5: while a campaign Co-Op watch is open, do not draw the normal
     * gameplay sight/ammo/radar afterward. Those late HUD passes were leaking
     * green glyphs along the bottom edge of every split-screen viewport. */
...
void bondviewKillCurrentPlayer(void)
{
    if ((g_CurrentPlayer->cheatBondInvincible == 0) && (g_CurrentPlayer->bonddead == FALSE))
    {
...
#ifdef GE_MODDED_CHEATS
        /* R21: Campaign Co-Op player death is per-player state.  The retail
         * global KIA latch is only valid once the shared mission lifecycle has
         * determined that the whole team has failed. */
        if (gamemode != GAMEMODE_MULTI || get_scenario() != SCENARIO_COOP)
        {
            g_isBondKIA = 1;
        }
...
#endif
#ifdef GE_MODDED_CHEATS
        /* RC6: campaign Co-Op deaths are counted at the instant the death is
         * committed, regardless of whether the source was a player, guard,
         * explosion or other mission hazard. The bonddead guard above makes
         * this exactly-once for the life. */
        if (gamemode == GAMEMODE_MULTI && get_scenario() == SCENARIO_COOP)
        {
         ...
         * Reuse two otherwise-unused retail BSS words so no player/BSS layout
         * is expanded. The timer measures the faster animation's duration;
         * after it reaches the last frame we hold for the same duration again,
         * reproducing the original half-speed collapse's total timing. */
        if (gamemode != GAMEMODE_MULTI && (g_ModGameplayOptions2 & MODOPT2_REALTIME_COLLAPSE))
        {
            g_Rc5RealtimeCollapsePhase = 0;
            g_Rc5RealtimeCollapseTimer = 0.0f;
...
void record_damage_kills(f32 damage_amount, f32 vectorx, f32 vectorz, s32 playerid, s32 affects_armor) {
...
#ifdef GE_MODDED_CHEATS
                    /*
                     * Source-level port of SubDrag/Zoinkity's USA Co-Op death
                     * patch (CoOp-USA.ips).  Campaign hazards/guards can pass a
                     * non-player owner such as -1.  Retail multiplayer later
                     * uses playerid as an array index and as set_cur_player(),
    ...
    #define ZERO_7F08991C 0
                ...
                if (g_CurrentPlayer->damageshowtime < ZERO_7F08991C
                    && !(g_ModGameplayOptions2 & MODOPT2_DISABLE_KNOCKBACK))
                {
...
void bondviewGetPropHeightRelatedValues(PropRecord *arg0, struct rect4f **field_B0, s32 *arg2, f32 *height_related, f32 *collision)
{
    ...
    if (g_playerPointers[temp_v0]->field_AC != 0)
    {
        ...
        if (getPlayerCount() == 1 || g_playerPointers[temp_v0]->bonddead == FALSE)
        {
            ...
            {
                struct player *player = g_playerPointers[temp_v0];

#ifdef GE_MODDED_CHEATS
                /*
                 * Campaign Co-Op can ask for another viewer's collision polygon
                 * before that viewer has reached its own movement update for the
                 * frame.  The retail code assumes the cached rectangle is already
                 * current; on a campaign setup with multiple Bonds that is not
                 * guaranteed.  Rebuild the ordinary player rectangle directly
                 * from the authoritative collision position/radius immediately
                 * before exposing it to stan collision code.  This closes the
                 * first-frame timing hole seen in the R8/R9 fault dumps.
                 *
                 * Valid retail multiplayer behaviour is unchanged because this
                 * is the same four-point rectangle written by
                 * bondviewUpdatePlayerCollisionBounds().
                 */
                if (gamemode == GAMEMODE_MULTI && get_scenario() == SCENARIO_COOP)
                {
                    /*
                     * Campaign setups spawn every Bond at the same single-player
                     * start pad.  While two player cylinders still intersect, do
                     * not expose the other Bond as a solid collision polygon.
                     * This lets the players walk apart naturally; as soon as the
                     * circles separate, normal player-vs-player collision turns
                     * itself back on automatically.
                     */
                    if (player != g_CurrentPlayer)
                    {
                        f32 dx = player->field_488.collision_position.f[0] - g_CurrentPlayer->field_488.collision_position.f[0];
                        f32 dz = player->field_488.collision_position.f[2] - g_CurrentPlayer->field_488.collision_position.f[2];
                        f32 radius = player->field_488.collision_radius + g_CurrentPlayer->field_488.collision_radius;
                    ...
                    player->collision_bounds.f[0] = player->field_488.collision_position.f[0] + player->field_488.collision_radius;
                    player->collision_bounds.f[1] = player->field_488.collision_position.f[2];
                    player->collision_bounds.f[2] = player->field_488.collision_position.f[0];
                    player->collision_bounds.f[3] = player->field_488.collision_position.f[2] + player->field_488.collision_radius;
                    player->collision_bounds.f[4] = player->field_488.collision_position.f[0] - player->field_488.collision_radius;
                    player->collision_bounds.f[5] = player->field_488.collision_position.f[2];
                    player->collision_bounds.f[6] = player->field_488.collision_position.f[0];
                    player->collision_bounds.f[7] = player->field_488.collision_position.f[2] - player->field_488.collision_radius;
                }
#endif

                *arg2 = 4;
                *field_B0 = &player->collision_bounds;
                *collision = player->field_70;
                *height_related = *collision + bondviewGetPlayerDuckingHeightRelated(player) + 10.0f;
...
Gfx* hudmsgBottomRender(Gfx* arg0)
{
    ...
    if ((g_CurrentPlayer->hudmessoff == FALSE) && (g_CurrentPlayer->bondmesscnt >= 0) && (g_CurrentPlayer->mpmenuon == FALSE))
    {
        ...
        if (var_v1 != 0)
        {
            ...
            view_top_offset = 0;
#ifdef GE_MODDED_CHEATS
            /* RC5: quarter-screen pickup/objective text uses tighter native
             * Bank Gothic tracking so messages fit the small viewport cleanly. */
            if (getPlayerCount() > 1)
            {
                setTextSpacingInverted(2);
            }
#endif
...
{
#ifdef GE_MODDED_CHEATS
    /* RC5: the upper-dialogue queue/timer is global, not per-player.  In
     * campaign Co-Op this routine is reached once for every viewport, which
     * made the dialogue expire 2x/3x/4x faster.  Player 1 owns the single
     * timer tick while every viewport still renders the same global message. */
    if (gamemode == GAMEMODE_MULTI && get_scenario() == SCENARIO_COOP
        && get_cur_playernum() != 0)
    {
        return;
...
Gfx *sub_GAME_7F08AAE8(Gfx *gdl)
{
    ...
    struct fontchar *msgchars;
    struct font *msgfont;
#ifdef GE_MODDED_CHEATS
    char wrappedmsg[0x100];
    char *msgtext;
#endif
        ...
        if (upper_text_window_timer >= 0)
        {
...
#endif
            {
                if (g_CurrentPlayer->mpmenuon == 0)
                {
                    ...
                    if (getPlayerCount() > 1)
                    {
                        msgchars = ptrFontBankGothicChars;
                        msgfont = ptrFontBankGothic;
                        setTextSpacingInverted(1);
                    }
                    else
                    {
                        msgchars = ptrFontZurichBoldChars;
                        msgfont = ptrFontZurichBold;
                    }
#if defined(LEFTOVERDEBUG)
#ifdef GE_MODDED_CHEATS
                    msgtext = stringbuffer_top[upper_text_buffer_index];
#else
                    textMeasure(&msg.textheight, &msg.textwidth, stringbuffer_top[upper_text_buffer_index], msgchars, msgfont, 0);
#endif
#else
#ifdef GE_MODDED_CHEATS
                    msgtext = dword_CODE_bss_80079DC8[upper_text_buffer_index];
#else
                    textMeasure(&msg.textheight, &msg.textwidth, dword_CODE_bss_80079DC8[upper_text_buffer_index], msgchars, msgfont, 0);
#endif
#endif
#ifdef GE_MODDED_CHEATS
                    if (getPlayerCount() > 1)
                    {
                        /* Wrap to the actual viewport instead of measuring one
                         * long line against the full framebuffer. */
                        textWrap(viGetViewWidth() - 12, msgtext, wrappedmsg, msgchars, msgfont);
                        msgtext = wrappedmsg;
                    }
                    textMeasure(&msg.textheight, &msg.textwidth, msgtext, msgchars, msgfont, 0);
#endif
                    if (cameraBufferToggle != 0)
                    {
                    ...
                    {
                        msg.x = viGetViewLeft() + (getPlayerCount() > 1 ? 6 : 0x1e);
#ifdef VERSION_EU
                        msg.y = viGetViewTop() + (getPlayerCount() > 1 ? 5 : 0x10);
#else
                        msg.y = viGetViewTop() + (getPlayerCount() > 1 ? 5 : 0xd);
#endif
                    }
                    ...
                    if (getPlayerCount() > 1)
                    {
                        gdl = microcode_constructor_related_to_menus(gdl, msg.x - 1, msg.y - 1, msg.x + msg.textwidth + 1, msg.bottom, 0x64);
                    }
                    else
                    {
...
#ifdef GE_MODDED_CHEATS
                    gdl = textRender(gdl, &msg.x, &msg.y, msgtext, msgchars, msgfont, -1, sw.screenwidth, viGetY(), 0, 0);
#else
                    gdl = textRender(gdl, &msg.x, &msg.y, stringbuffer_top[upper_text_buffer_index], msgchars, msgfont, -1, sw.screenwidth, viGetY(), 0, 0);
#endif
#else
                    if (j_text_trigger != 0)
                    {
                        sw.screenwidth = viGetX();
#ifdef GE_MODDED_CHEATS
                        gdl = textRenderOutlined(gdl, &msg.x, &msg.y, msgtext, msgchars, msgfont, -1, 0x646464FF, sw.screenwidth, viGetY(), 0, 0);
#elif defined(LEFTOVERDEBUG)
                        gdl = textRenderOutlined(gdl, &msg.x, &msg.y, stringbuffer_top[upper_text_buffer_index], msgchars, msgfont, -1, 0x646464FF, sw.screenwidth, viGetY(), 0, 0);
#else
                        gdl = textRenderOutlined(gdl, &msg.x, &msg.y, dword_CODE_bss_80079DC8[upper_text_buffer_index], msgchars, msgfont, -1, 0x646464FF, sw.screenwidth, viGetY(), 0, 0);
#endif
                    }
                    else
                    {
                        ...
                        gdl = textRender(gdl, &msg.x, &msg.y, stringbuffer_top[upper_text_buffer_index], msgchars, msgfont, -1, sw.screenwidth, viGetY(), 0, 0);
#else
                        gdl = textRender(gdl, &msg.x, &msg.y, dword_CODE_bss_80079DC8[upper_text_buffer_index], msgchars, msgfont, -1, sw.screenwidth, viGetY(), 0, 0);
#endif
                    }
#endif
                    if (getPlayerCount() > 1)
