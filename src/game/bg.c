...
/*
 * Physical optimized build: per-level room/portal adjacency cache.
 *
 * Keep optimization-only scratch state in upper Expansion Pak RDRAM rather
 * than moving GoldenEye's original lower-4-MiB .bss layout.  ge007.ld places
 * _gameFastBssStart immediately after resident physical game text.
 */
#define BG_FAST_ROOM_LINK_WORDS ((MAXROOMCOUNT + 31) / 32)
struct BgFastPortalCache
{
...
static void bgBuildPortalFastCache(void)
{
    ...
    for (room = 0; room < MAXROOMCOUNT; room++)
    {
        ...
        {
            g_BgFastRoomLinks[room][word] = 0;
        }
    }

    for (portal = 0; portal < PORTMAX * 2; portal++)
    {
        g_BgFastPortalNext[portal] = -1;
    }

    g_BgFastPortalCount = 0;

    while (g_BgFastPortalCount < PORTMAX &&
           g_BgPortals[g_BgFastPortalCount].offset_portal != NULL)
    {
        s32 p = g_BgFastPortalCount;
        s32 room1 = g_BgPortals[p].connectedRoom1;
        s32 room2 = g_BgPortals[p].connectedRoom2;
        s32 node1 = p * 2;
        s32 node2 = node1 + 1;

        if ((u32) room1 < MAXROOMCOUNT)
        {
            if (g_BgFastRoomPortalTail[room1] >= 0)
            {
                g_BgFastPortalNext[g_BgFastRoomPortalTail[room1]] = node1;
            }
            else
            {
                g_BgFastRoomPortalHead[room1] = node1;
            }
            g_BgFastRoomPortalTail[room1] = node1;

            if ((u32) room2 < MAXROOMCOUNT)
            {
                g_BgFastRoomLinks[room1][room2 >> 5] |= 1u << (room2 & 31);
            }
        }

        if ((u32) room2 < MAXROOMCOUNT)
        {
            if (g_BgFastRoomPortalTail[room2] >= 0)
            {
                g_BgFastPortalNext[g_BgFastRoomPortalTail[room2]] = node2;
            }
            else
            {
                g_BgFastRoomPortalHead[room2] = node2;
            }
            g_BgFastRoomPortalTail[room2] = node2;

            if ((u32) room1 < MAXROOMCOUNT)
            {
                g_BgFastRoomLinks[room2][room1 >> 5] |= 1u << (room1 & 31);
            }
        }
...
static s32 bgFastRoomFirstPortalNode(s32 room)
{
    ...
    return g_BgFastRoomPortalHead[room];
}
#endif
...
#ifdef GE_PHYSICAL_FASTPATHS
static void bgBuildPortalMetricCache(void);
#endif
...
#ifdef GE_PHYSICAL_FASTPATHS
        /*
         * The upper-RDRAM portal cache survives warm boots/stage changes and
         * is not valid for this BG yet.  Both sub_GAME_7F0B993C and
         * bgOrderPortal call sub_GAME_7F0B96CC, which normally consults that
         * cache.  Force raw portal metrics until ordering is complete; using
         * a stale previous-stage metric here can swap room endpoints and make
         * the starting room render in isolation until the level is reloaded.
         */
        g_BgFastPortalCount = 0;
#endif
...
#ifdef GE_PHYSICAL_FASTPATHS
        /*
         * Build directional adjacency only after bgOrderPortal has finished
         * swapping portal endpoints.  The R5 prototype built this too early,
         * which could leave node-side orientation stale even though the
         * symmetric room-link bitset remained correct.
         */
...
void bgRoomVisibilityRelated(void)
{
    ...
    for (depth = 0, maxdepth = 11; depth != maxdepth; depth++) {
#ifdef GE_PHYSICAL_FASTPATHS
        {
            s32 node = bgFastRoomFirstPortalNode(room);
            portalnum = g_BgFastPortalCount;

            while (node >= 0)
            {
                s32 p = node >> 1;

                if (D_800443C4[p] == 0 && p != lastportal && sub_GAME_7F0B9F14(p, pos, pos3))
                {
                    portalnum = p;
                    lastportal = p;
                    room = (room ^ g_BgPortals[p].connectedRoom1) ^ g_BgPortals[p].connectedRoom2;
                    break;
                }
        ...
        if (portalnum >= g_BgFastPortalCount)
        {
            break;
        }
#else
...
void bgFindRoomsAlongSegment(coord3d *fromPos, coord3d *toPos, u8 *fromRooms, u8 *finalRooms, s32 *traversedRooms, s32 *traversedRoomCount, s32 maxTraversedRooms)
{
    ...
    do
    {
        ...
        {
#ifdef GE_PHYSICAL_FASTPATHS
            u8 status = portalStatuses[portalnum];

            if (status != 0)
            {
                u8 sourceRoom = status == 1
                    ? g_BgPortals[portalnum].connectedRoom1
                    : g_BgPortals[portalnum].connectedRoom2;
                u8 destRoom = status == 1
                    ? g_BgPortals[portalnum].connectedRoom2
                    : g_BgPortals[portalnum].connectedRoom1;

                for (i = 0; (sourceRooms[i] != 0xff) && (i < 16); i++)
                {
                    if (sourceRooms[i] == sourceRoom)
                    {
                        addToByteSetMaxSize15(foundRooms, destRoom);
                        addToByteSetMaxSize15(allRooms, destRoom);
...
void bgLoadRoomModelData(s32 roomID)
{
    ...
    */
     ...
     */
    if (!loadfailed && g_BgRoomInfo[roomID].csize_primary_DL_binary)
    {
     ...
     */
    if (!loadfailed && g_BgRoomInfo[roomID].csize_secondary_DL_binary)
    {
    ...
    else if (!g_BgRoomInfo[roomID].csize_secondary_DL_binary)
    {
        g_BgRoomInfo[roomID].ptr_secondary_expanded_mapping_info = NULL;
    }

#ifdef GE_MODDED_CHEATS
    /* Never poison a room as "loaded" after a short/fragmented allocation.
     * RC6 could leave model_bin_loaded=1 with missing GDL pointers, making an
     * intermittent cold-load black room persist for the rest of the stage.
     * Free the partial block and leave the room unloaded so the next visible
     * pass can GC/defrag and retry it normally. */
    if (loadfailed)
    {
...
#ifdef GE_PHYSICAL_FASTPATHS
    {
        s32 node = bgFastRoomFirstPortalNode(otherroom);
...
#ifdef GE_PHYSICAL_FASTPATHS
    for (i = 0; i < g_BgFastPortalCount; i++)
#else
    for (i = 0; i < PORTMAX; i++)
#endif
    {
...
s32 bgGetConnectedRooms(s32 roomIndex, s32* list, s32 max)
{
    ...
    if ((u32) roomIndex < MAXROOMCOUNT)
    {
        node = g_BgFastRoomPortalHead[roomIndex];

        while (node >= 0)
        {
            s32 p = node >> 1;
            s32 connectedRoom = (node & 1)
                ? g_BgPortals[p].connectedRoom1
                : g_BgPortals[p].connectedRoom2;

            for (i = 0; i < len; i++)
            {
                if (list[i] == connectedRoom)
                {
                    break;
                }
            }
...
#ifdef GE_PHYSICAL_FASTPATHS
    /* Out-of-range room IDs retain retail's scan semantics. */
    {
        s32 p;
        s32 connectedRoom1;
        s32 connectedRoom2;
...
{
#ifdef GE_PHYSICAL_FASTPATHS
    if ((u32) room1 < MAXROOMCOUNT && (u32) room2 < MAXROOMCOUNT)
    {
        return (g_BgFastRoomLinks[room1][room2 >> 5] & (1u << (room2 & 31))) != 0;
    }
#endif
    {
        s32 i;
...
void bgUpdateCurrentPlayerScreenMinMax(void)
{
    /* R21 RC9: backport Perfect Dark's per-player viewport bounds semantics.
     *
     * The old GoldenEye path clamps through the global bgViewRelated[] inset.
     * That state is not a player viewport and is a bad ownership boundary for
     * 3/4-player rendering.  Derive the visibility rectangle only from the
     * current player's VI viewport, then clamp it to the framebuffer exactly
     * like Perfect Dark does.  No view-dependent result is cached globally. */
    f32 width = (f32)viGetX();
    f32 height = (f32)viGetY();

    g_CurrentPlayer->screensize.min.x = (f32)viGetViewLeft();
    if (g_CurrentPlayer->screensize.min.x < 0.0f)
        g_CurrentPlayer->screensize.min.x = 0.0f;
    if (g_CurrentPlayer->screensize.min.x > width)
        g_CurrentPlayer->screensize.min.x = width;

    g_CurrentPlayer->screensize.min.y = (f32)viGetViewTop();
    if (g_CurrentPlayer->screensize.min.y < 0.0f)
        g_CurrentPlayer->screensize.min.y = 0.0f;
    if (g_CurrentPlayer->screensize.min.y > height)
        g_CurrentPlayer->screensize.min.y = height;

    g_CurrentPlayer->screensize.max.x = (f32)(viGetViewLeft() + viGetViewWidth());
    if (g_CurrentPlayer->screensize.max.x < 0.0f)
        g_CurrentPlayer->screensize.max.x = 0.0f;
    if (g_CurrentPlayer->screensize.max.x > width)
        g_CurrentPlayer->screensize.max.x = width;

    g_CurrentPlayer->screensize.max.y = (f32)(viGetViewTop() + viGetViewHeight());
    if (g_CurrentPlayer->screensize.max.y < 0.0f)
        g_CurrentPlayer->screensize.max.y = 0.0f;
    if (g_CurrentPlayer->screensize.max.y > height)
        g_CurrentPlayer->screensize.max.y = height;
}
...
#ifdef GE_PHYSICAL_FASTPATHS
    {
        s32 node = bgFastRoomFirstPortalNode(roomID);
...
//#endif
static void bgComputePortalMetricRaw(s32 portalnum, f32 *out)
{
    f32 sp6c[3];
    f32 sp60[3];
...
static void bgBuildPortalMetricCache(void)
{
    ...
    for (portal = 0; portal < g_BgFastPortalCount; portal++)
    {
        bgComputePortalMetricRaw(portal, (f32 *) &g_BgFastPortalMetrics[portal]);
    }
}
#endif
...
{
#ifdef GE_PHYSICAL_FASTPATHS
    if ((u32) portalnum < (u32) g_BgFastPortalCount)
    {
        struct PortalMetric *metric = &g_BgFastPortalMetrics[portalnum];
        out[0] = metric->normal.f[0];
        out[1] = metric->normal.f[1];
        out[2] = metric->normal.f[2];
        out[3] = metric->min;
        out[4] = metric->max;
        return;
    }
#endif
    bgComputePortalMetricRaw(portalnum, out);
}
...
s32 bgGetPortalBetweenRooms(s32 room1, s32 room2, coord3d *arg2, coord3d *arg3)
{
    ...
    if ((u32) room1 < MAXROOMCOUNT)
    {
        s32 node = g_BgFastRoomPortalHead[room1];

        while (node >= 0)
        {
            s32 i = node >> 1;
            s32 other = (node & 1)
                ? g_BgPortals[i].connectedRoom1
                : g_BgPortals[i].connectedRoom2;
            ...
            if (other == room2)
            {
                ...
                {
                    if (portalIndex >= 0) osSyncPrintf("bg: bgGetPortalBetweenRooms(): Multiple portals join room '%s' and '%s'\\n", bgDebPrintROOMID(room1), bgDebPrintROOMID(room2));
                    portalIndex = i;
                }
            }
