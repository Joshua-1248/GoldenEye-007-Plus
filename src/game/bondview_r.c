        ...
        while (intro_record->type != INTROTYPE_END)
        {
            switch (intro_record->type)
            {
                ...
                case INTROTYPE_SWIRL:
                {
                    ...
                    /* Campaign intros were written for one player.  In Co-Op
                     * this routine runs once per player, so converting the same
                     * in-place integer swirl values again corrupts them. */
                    if (get_scenario() != SCENARIO_COOP || get_cur_playernum() == 0)
#endif
                    {
