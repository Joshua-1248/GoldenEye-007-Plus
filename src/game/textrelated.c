...
extern u8 _fontCacheStart[];
extern u8 _fontCacheEnd[];
#define FONT_CACHE_MAGIC 0x4745464eU /* "GEFN" */
#endif
...
#ifdef GE_PHYSICAL_FASTPATHS
    {
        u32 gothiclen = (romptr_t)&_fontbankgothicSegmentEnd - (romptr_t)&_fontbankgothicSegmentStart;
        u32 zurichlen = (romptr_t)&_fontzurichboldSegmentEnd - (romptr_t)&_fontzurichboldSegmentStart;
        u8 *cache = _fontCacheStart;
        u8 *gothic = (u8 *)(((uintptr_t)(cache + 16) + 15) & ~15);
        u8 *zurich = (u8 *)(((uintptr_t)(gothic + gothiclen) + 15) & ~15);
        u32 *magic = (u32 *)cache;

        /* Preserve the retail stage-bank allocation footprint so unrelated
         * stage allocations retain their original ordering/addresses. */
        mempAllocBytesInBank(gothiclen, MEMPOOL_STAGE);
        mempAllocBytesInBank(zurichlen, MEMPOOL_STAGE);

        if (zurich + zurichlen <= _fontCacheEnd)
        {
            ptrFontBankGothic = (struct font *)gothic;
            ptrFontZurichBold = (struct font *)zurich;

            if (magic[0] != FONT_CACHE_MAGIC || magic[1] != ~FONT_CACHE_MAGIC)
            {
                romCopy(ptrFontBankGothic, (void *)&_fontbankgothicSegmentRomStart, gothiclen);
                ...
                romCopy(ptrFontZurichBold, (void *)&_fontzurichboldSegmentRomStart, zurichlen);
                ...
                magic[0] = FONT_CACHE_MAGIC;
                magic[1] = ~FONT_CACHE_MAGIC;
            }
            else
            {
                ptrFontBankGothicChars = ptrFontBankGothic->chars;
