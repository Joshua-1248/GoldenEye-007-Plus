...
void bossMainloop(void)
{
    ...
    while (!done)
    {
        ...
        if (g_DebugAndUpdateStageFlag)
        {
            ...
            if (g_StageNum != LEVELID_TITLE && gamemode == GAMEMODE_MULTI && get_scenario() != SCENARIO_COOP)
#else
    ...
    */
                    ...
                    else
                    {
                        if (g_MainStageNum < 0 && pendingGfx < 2U)
                        {
                            ...
                            if (g_StageNum != LEVELID_TITLE)
                            {
                                /* R18: keep ticking every Co-Op player during an exit cinema.
                                 * R15 also collapsed the simulation/update loop to P1, but GE's
                                 * multiplayer lifecycle still expects each live player to advance
                                 * its fade/body/camera bookkeeping before the stage is torn down.
                                 * Perfect Dark's shared-view Co-Op path similarly treats
                                 * single-screen cutscenes as a presentation rule, not a reduced
                                 * player-count simulation.  lvlRender() remains P1-only while the
                                 * end-cinema shared viewport is active. */
...
#endif
#ifdef GE_MODDED_CHEATS
    /* RC6: snapshot campaign Co-Op before any title-stage transition can
     * dismantle/reset multiplayer state. This covers both natural POSEND
     * completion and player-skipped exit cinematics. */
