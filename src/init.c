...
#define RZIPLOADADDR 0x80200000
#else
...
#ifdef GE_PHYSICAL_CODE
    /*
     * The retail game segment is demand-paged from ROM into a 90-page TLB
     * cache.  Physical-code mode instead places the whole segment in KSEG0
     * and performs one startup DMA.  All game function pointers/jumps are
     * linked directly against this resident address range.
     */
    {
        u32 gameSegmentSize = (u32)(_gameSegmentEnd - _gameSegmentStart);

        /* Physical-code builds intentionally use Expansion Pak RDRAM.
         * Never issue a PI DMA into nonexistent memory on a 4 MiB console. */
        ...
        osInvalDCache(_gameSegmentStart, gameSegmentSize);
        osPiRawStartDma(OS_READ, &_gameSegmentRomStart, _gameSegmentStart, gameSegmentSize);

        while (osPiGetStatus() & PI_STATUS_DMA_BUSY)
        {
        }

        /* PI DMA bypasses the CPU caches.  No page refill or TLB handler is
         * needed after this point. */
        osInvalDCache(_gameSegmentStart, gameSegmentSize);
        osInvalICache(_gameSegmentStart, gameSegmentSize);
    }
#else
