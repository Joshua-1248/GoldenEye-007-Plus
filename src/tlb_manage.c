...
{
#ifdef GE_PHYSICAL_CODE
    /* No software code-page cache in the physical build.  Preserve the
     * retail heap ceiling exactly for the first physical-code baseline: the
     * former 90-page cache begins at 0x802F6000.  Game code itself lives in
     * Expansion Pak RDRAM, so the original 4 MiB allocation map is untouched. */
    g_tlbCurrentEntryCount = 0;
    g_tlbSegmentIndex = 0;
    g_tlbmanageTlbAllocatedBlock = (u8(*)[TLB_BLOCK_SIZE])
        (((u32)&sp_boot & ~(PAGE_SIZE - 1)) - (MAPPING_TABLE_COUNT * PAGE_SIZE));
    g_tlbmanageMappingTableEnd = 0;
#else
    s32 i;
...
{
#ifdef GE_PHYSICAL_CODE
    /* The physical build never installs game-code TLB mappings.  Keeping this
     * as an explicit no-op removes the last compiled 0x7F000000 executable
     * assumption from the physical resident code and makes accidental calls
     * harmless even though the retail mapping tables are intentionally not
     * initialized. */
    (void)index;
#else
    s32 tlbIndex;
...
void tlbmanageTranslateLoadRomFromTlbAddress(u32 address)
{
    ...
    /*
     * A second thread can already be queued on the same code-page fault while
     * the first fault is serviced.  Retail blindly DMA-copies another 8 KiB
     * copy when that stale fault is later dequeued, creating duplicate cache
     * entries.  If the software page table is now valid, the page has already
     * been loaded; requeueing the thread lets the normal low-level refill
     * vector install the hardware TLB entry on retry.
     */
    maskedAddress = alignedAddress & 0xFFE000;
    tlbIndex = GET_TLB_INDEX_FROM_MASK(maskedAddress);

    if (g_tlbManagementTable[tlbIndex].contextValue != 1)
    {
        return;
    }
#endif
...
#if defined(GE_OPTIMIZED)
    // Either retail call independently invalidates the complete 16 KiB I-cache
    // (nbytes >= ICACHE_SIZE). One full pass leaves the same final cache state.
    osInvalICache((void *)0x80000000, 0x10000000);
#else
    // Retail sequence retained byte-for-byte in the matching build.
